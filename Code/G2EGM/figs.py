...
def decision_functions(model, t):
    ...
    # c. varnames
    for varname in ["c", "d", "inv_v", "w", "wa", "wb"]:
        try:
            if "w" in varname:
                if t == par.T - 1:
                    continue
...
def segments(model, t):
    ...
    if Icon.sum() > 0:
        ax.scatter(m[Icon], n[Icon], s=4, color=color3, label="con")
    if Iacon.sum() > 0:
        ax.scatter(m[Iacon], n[Iacon], s=4, color=color1, label="acon")
    if Idcon.sum() > 0:
        ax.scatter(m[Idcon], n[Idcon], s=4, color=color2, label="dcon")
    if Iucon.sum() > 0:
        ax.scatter(m[Iucon], n[Iucon], s=4, color="black", label="ucon")
