...
def solve(sol, par, G2EGM=True):
    ...
    for j in range(2, par.T + 1):
        ...
        linear_interp.interp_1d_vec(
            sol.m_ret[t + 1, :], sol.c_ret[t + 1, :], m_plus, c_plus
        )
        ...
        # ii. constraint
        sol.c_ret[t, : par.Nmcon_ret] = nonlinspace_jit(
            1e-6, sol.c_ret[t, par.Nmcon_ret] * 0.999, par.Nmcon_ret, par.phi_m
        )
        ...
        linear_interp.interp_1d_vec(
            sol.m_ret[t + 1, :], sol.inv_v_ret[t + 1, :], m_plus, inv_v_plus
        )
