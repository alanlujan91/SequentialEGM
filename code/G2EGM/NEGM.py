...
from consav import golden_section_search, upperenvelope
...
def solve_pure_c(t, sol, par):
    ...
    for i_b in range(par.Nb_pd):
        ...
        # upperenvelope
        negm_upperenvelope(
            par.grid_a_pd, temp_m, temp_c, w[i_b], par.grid_l, c[i_b, :], temp_v, par
        )
...
def obj_outer(d, n, m, t, sol, par):
    ...
    return -linear_interp.interp_2d(
        par.grid_b_pd, par.grid_l, sol.inv_v_pure_c[t], n_pure_c, m_pure_c
    )  # we are minimizing
...
def solve_outer(t, sol, par):
    ...
    for i_n in range(par.Nn):
        ...
        for i_m in range(par.Nm):
            ...
            d_low = 1e-8
            d_high = m - 1e-8
            d[i_n, i_m] = golden_section_search.optimizer(
                obj_outer, d_low, d_high, args=(n, m, t, sol, par), tol=1e-8
            )
            ...
            c[i_n, i_m] = np.fmin(
                linear_interp.interp_2d(
                    par.grid_b_pd, par.grid_l, sol.c_pure_c[t], n_pure_c, m_pure_c
                ),
                m_pure_c,
            )
            ...
            if obj_dcon > inv_v[i_n, i_m]:
                c[i_n, i_m] = linear_interp.interp_2d(
                    par.grid_b_pd, par.grid_l, sol.c_pure_c[t], n, m
                )
                d[i_n, i_m] = 0
