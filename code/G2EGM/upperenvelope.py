...
@njit
def upperenvelope(
    out_c, out_d, out_v, holes, i_a, i_b, tri, m, n, c, d, v, Na, Nb, valid, num, w, par
):
    ...
    for i_n in range(in_low, in_high):
        for i_m in range(im_low, im_high):
            ...
            if num == 1:  # ucon, interpolate c and d
                c_interp = (
                    w1 * c[i_b_1, i_a_1] + w2 * c[i_b_2, i_a_2] + w3 * c[i_b_3, i_a_3]
                )
                d_interp = (
                    w1 * d[i_b_1, i_a_1] + w2 * d[i_b_2, i_a_2] + w3 * d[i_b_3, i_a_3]
                )
            ...
            # v. value-of-choice
            w_interp = linear_interp.interp_2d(
                par.grid_b_pd, par.grid_a_pd, w, b_interp, a_interp
            )
