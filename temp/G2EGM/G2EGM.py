...
def deviate_d_con(valt, n, c, a, w, par):
    for i_b in range(par.Nb_pd):
        for i_a in range(par.Na_pd):
            ...
            # c. value
            w_x = linear_interp.interp_2d(
                par.grid_b_pd, par.grid_a_pd, w, b_x, a[i_b, i_a]
            )
...
def solve_acon(out_c, out_d, out_v, w, wb, par):
    ...
    for i_b in range(par.Nb_acon):
        # ii. setup
        wb_acon = linear_interp.interp_2d(
            par.grid_b_pd, par.grid_a_pd, wb, par.b_acon[i_b], 0
        )
        ...
        w_acon[:, i_b] = linear_interp.interp_2d(
            par.grid_b_pd, par.grid_a_pd, w, par.b_acon[i_b], 0
        )
...
def solve_con(out_c, out_d, out_v, w, par):
    ...
    linear_interp.interp_2d_vec(
        par.grid_b_pd, par.grid_a_pd, w, b.ravel(), a.ravel(), w_con.ravel()
    )
...
def solve(t, sol, par):
    ...
    solve_ucon(
        sol.ucon_c[t, :, :], sol.ucon_d[t, :, :], sol.ucon_v[t, :, :], w, wa, wb, par
    )
    solve_dcon(
        sol.dcon_c[t, :, :], sol.dcon_d[t, :, :], sol.dcon_v[t, :, :], w, wa, par
    )
    solve_acon(
        sol.acon_c[t, :, :], sol.acon_d[t, :, :], sol.acon_v[t, :, :], w, wb, par
    )
    ...
    linear_interp.interp_2d_vec(
        par.grid_b_pd, par.grid_a_pd, wb, b.ravel(), a.ravel(), wb_now.ravel()
    )
