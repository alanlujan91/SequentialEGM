...
base_params["pLvlInitMean"] = np.log(
    23.72
)  # From Table 1, in thousands of USD (Q-I: where is this from?)
...
def CheckErgodicDistribution(CheckType, Ergodic_Tol):
    ...
    Perc_Change_Mean_aNrmNow = (
        100
        * abs(Mean_aNrmNow[1 : CheckType.T_sim] - Mean_aNrmNow[0 : CheckType.T_sim - 1])
        / Mean_aNrmNow[1 : CheckType.T_sim]
    )
    Perc_Change_Std_aNrmNow = (
        100
        * abs(Std_aNrmNow[1 : CheckType.T_sim] - Std_aNrmNow[0 : CheckType.T_sim - 1])
        / Std_aNrmNow[1 : CheckType.T_sim]
    )

    Periods_Above_Tol = np.argwhere(Perc_Change_Mean_aNrmNow > Ergodic_Tol) + 2
    LastPeriod_Above_Tol = int(max(Periods_Above_Tol))
    if (
        LastPeriod_Above_Tol < CheckType.T_sim - 50
    ):  # Last period should at least be 50 periods before end
        print(
            "The simulation required ",
            LastPeriod_Above_Tol,
            " periods for the change in mean wealth to become permanently smaller than ",
            Ergodic_Tol,
            "%.",
        )
    else:
        print(
            "The simulation never reached a stable mean wealth value below the imposed tolerance of ",
            Ergodic_Tol,
            "%.",
        )
    ...
    if LastPeriod_Above_Tol < CheckType.T_sim - 50:
        print(
            "The simulation required ",
            LastPeriod_Above_Tol,
            " periods for the change in the standard deviation of wealth to become permanently smaller than ",
            Ergodic_Tol,
            "%.",
        )
    else:
        print(
            "The simulation never reached a stable standard deviation of wealth value below the imposed tolerance of ",
            Ergodic_Tol,
            "%.",
        )
...
def FagerengObjFunc(
    SplurgeEstimate, center, spread, verbose=False, estimation_mode=True
):
    """
...
[
    ...
    Splurge, beta_dist_estimate[0], beta_dist_estimate[1], estimation_mode=False
)
...
[
    ...
    opt_splurge, beta_dist_estimate[0], beta_dist_estimate[1], estimation_mode=False
)
print(
    "Agg MPC from first year to year t+4 \n", simulated_MPC_mean_add_Lottery_Bin, "\n"
)  # %% Plot aggregate MPC and MPCX
...
[
    ...
    T_hist,
    P_hist,
] = FagerengObjFunc(opt[0], opt[1], opt[2], estimation_mode=False)
print(
    "Agg MPC from first year to year t+4 \n", simulated_MPC_mean_add_Lottery_Bin, "\n"
)  # %% Plot aggregate MPC and MPCX
print("Distance for Agg MPC is", distance_Agg_MPC, "\n")
...
[
    ...
    T_hist,
    P_hist,
] = FagerengObjFunc(Splurge, opt_params[0], opt_params[1], estimation_mode=False)
print(
    "Agg MPC from first year to year t+4 \n", simulated_MPC_mean_add_Lottery_Bin, "\n"
)  # %% Plot aggregate MPC and MPCX
print("Distance for Agg MPC is", distance_Agg_MPC, "\n")
