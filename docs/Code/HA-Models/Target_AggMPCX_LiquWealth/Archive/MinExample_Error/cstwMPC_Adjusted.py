    ...
    def updateIncomeProcess(self):
        ...
        if self.cycles == 0:
            ...
            TranShkDstn = deepcopy(
                approxMeanOneLognormal(
                    self.TranShkCount, sigma=self.TranShkStd[0], tail_N=0
                )
            )
            TranShkDstn.pmf = np.insert(
                TranShkDstn.pmf * (1.0 - self.UnempPrb), 0, self.UnempPrb
            )
            TranShkDstn.X = np.insert(
                TranShkDstn.X * (1.0 - tax_rate) * self.IndL, 0, self.IncUnemp
            )
            PermShkDstn = approxMeanOneLognormal(
                self.PermShkCount, sigma=self.PermShkStd[0], tail_N=0
            )
...
class cstwMPCmarket(EstimationMarketClass):
    ...
    def calcStats(
        ...
        if LorenzBool:
            ...
            if ManyStatsBool:
                self.LorenzLong = getLorenzShares(
                    aLvl,
                    weights=CohortWeight,
    ...
    def distributeParams(self, param_name, param_count, center, spread, dist_type):
        ...
        if dist_type == "uniform":
            ...
            param_dist = approxUniform(
                N=param_count, bot=center - spread, top=center + spread
            )
        elif dist_type == "lognormal":
            # If lognormal, center is the mean and spread is the standard deviation (in log)
            tail_N = 3
            param_dist = approxLognormal(
                N=param_count - tail_N,
        ...
        while j < len(self.agents):
            for n in range(replication_factor):
                ...
                exec("self.agents[j](" + param_name + "= param_dist.X[b])")
                j += 1
            b += 1
    ...
    def calcLorenzDistance(self):
        ...
        """
        LorenzSim = np.mean(
            np.array(self.Lorenz_hist)[self.ignore_periods :, :], axis=0
        )
    ...
    def showManyStats(self, spec_name=None):
        ...
        MPCbyIncome = np.mean(
            np.array(self.MPCbyIncome_hist)[self.ignore_periods :, :], axis=0
        )
        MPCbyWealthRatio = np.mean(
            np.array(self.MPCbyWealthRatio_hist)[self.ignore_periods :, :], axis=0
        )
        HandToMouthPct = np.mean(
            np.array(self.HandToMouthPct_hist)[self.ignore_periods :, :], axis=0
        )
        ...
        LorenzSim = np.hstack(
            (
                np.array(0.0),
                np.mean(
                    np.array(self.LorenzLong_hist)[self.ignore_periods :, :], axis=0
                ),
                np.array(1.0),
            )
        )
...
def getKYratioDifference(Economy, param_name, param_count, center, spread, dist_type):
    ...
    Economy.distributeParams(
        param_name, param_count, center, spread, dist_type
    )  # Distribute parameters
    Economy.solve()
    diff = Economy.calcKYratioDifference()
    print(
        "getKYratioDifference tried center = " + str(center) + " and got " + str(diff)
    )
    return diff
...
def findLorenzDistanceAtTargetKY(
    ...
    intermediateObjective = lambda center: getKYratioDifference(
        Economy=Economy,
    ...
    print(
        "findLorenzDistanceAtTargetKY tried spread = "
        + str(spread)
        + " and got "
        + str(dist)
    )
