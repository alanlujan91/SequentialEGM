...
aXtraNestFac = (
    3  # Number of times to 'exponentially nest' when constructing assets grid
)
...
pop_sim_agg_point = (
    9600  # Total simulated population size, with aggregate shocks, param-point model
)
pop_sim_agg_dist = (
    16800  # Total simulated population size, with aggregate shocks, param-dist model
)
pop_sim_ind_point = (
    10000  # Total simulated population size, no aggregate shocks, param-point model
)
pop_sim_ind_dist = (
    14000  # Total simulated population size, no aggregate shocks, param-dist model
)
T_sim_PY = (
    1200  # Number of periods to simulate (idiosyncratic shocks model, perpetual youth)
)
...
ignore_periods_PY = (
    400  # Number of periods to throw out when looking at history (perpetual youth)
)
ignore_periods_LC = (
    400  # Number of periods to throw out when looking at history (lifecycle)
)
...
TranShkStd = (
    np.concatenate(
        (
            np.linspace(0.1, 0.12, 17),
            0.12 * np.ones(17),
            np.linspace(0.12, 0.075, 61),
            np.linspace(0.074, 0.007, 68),
            np.zeros(retired_T + 1),
        )
    )
    * 4
) ** 0.5
TranShkStd = np.ndarray.tolist(TranShkStd)
PermShkStd = np.concatenate(
    (
        (
            (0.00011342 * (np.linspace(24, 64.75, working_T - 1) - 47) ** 2 + 0.01)
            / (11.0 / 4.0)
        )
        ** 0.5,
        np.zeros(retired_T + 1),
...
for j in range(
    len(d_death_probs)
):  # Convert annual mortality rates to quarterly survival rates
...
PermGroFac_d_base += 31 * [
    PermGroFac_d_base[-1]
]  # Add 31 years of the same permanent income growth rate to the end of the sequence
PermGroFac_h_base += 31 * [PermGroFac_h_base[-1]]
PermGroFac_c_base += 31 * [PermGroFac_c_base[-1]]
PermGroFac_d_retire = PermGroFac_d_base[
    40
]  # Store the big shock to permanent income at retirement
PermGroFac_h_retire = PermGroFac_h_base[40]
PermGroFac_c_retire = PermGroFac_c_base[40]
PermGroFac_d_base[40] = PermGroFac_d_base[
    39
]  # Overwrite the "retirement drop" with the adjacent growth rate
...
PermGroFac_c = []
for j in range(
    len(PermGroFac_d_base)
):  # Make sequences of quarterly permanent income growth factors from annual permanent income growth rates
...
PermGroFac_d[working_T - 1] = (
    1 + PermGroFac_d_retire
)  # Put the big shock at retirement back into the sequence
PermGroFac_h[working_T - 1] = 1 + PermGroFac_h_retire
PermGroFac_c[working_T - 1] = 1 + PermGroFac_c_retire

# Import the SCF wealth data
f = open(data_location + "/" + SCF_data_file)
...
init_market = {
    ...
    "Population": 0,  # Will get overwritten
    ...
    "AggShockBool": False,  # Will get overwritten
    "PermGroFacAgg": 1.0,
}
...
init_agg_shocks["MgridBase"] = np.array(
