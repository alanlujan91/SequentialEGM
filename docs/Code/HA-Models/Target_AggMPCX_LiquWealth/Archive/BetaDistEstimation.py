        ...
        for period in range(
            ...
            for k in range(N_Lottery_Win_Sizes):  # Loop through different lottery sizes
                ...
                if RandomLotteryWin and k == 5:
                    for i in range(ThisType.AgentCount):
                        Llvl[i] = (
                            lottery_size[random.randint(0, 3)] * LotteryWin[i, period]
                        )
                ...
                if period % 4 + 1 == 4:  # if we are in the 4th quarter of a year
                    ...
                    MPC_this_type[:, k, year - 1] = (
                        np.sum(c_actu_Lvl_year, axis=1)
                        - np.sum(c_base_Lvl_year, axis=1)
                    ) / (lottery_size[k])
...
def f_splurge(splurge, beta, nabla):
    ...
    opt_beta_nabla = minimizeNelderMead(
        f_beta_nabla, start_beta_nabla, verbose=True, xtol=1e-3, ftol=1e-3
    )
...
opt_splurge = minimizeNelderMead(
    f_temp, start_splurge, verbose=True, xtol=1e-3, ftol=1e-3
)
...
print(
    "Lorenz shares at 20th, 40th, 60th and 80th percentile",
    Lorenz_Data_Adj[20],
    Lorenz_Data_Adj[40],
    Lorenz_Data_Adj[60],
    Lorenz_Data_Adj[80],
...
print(
    "Percentile with zero cummulative assets",
    np.argwhere(Lorenz_Data > 0)[0] - 1,
    "% \n",
)
...
[
    ...
    opt[0], opt[1], opt[2], estimation_mode=False, target="AGG_MPC_plus_Liqu_Wealth"
)
