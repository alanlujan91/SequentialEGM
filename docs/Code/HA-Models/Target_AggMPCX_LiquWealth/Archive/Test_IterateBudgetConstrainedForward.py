...
for period in range(1, CheckType.T_sim):
    if AlwaysStartFromCorrect_a:
        if CheckType.TranShkNow_hist[period] != 1:
            m_adj = (
                CheckType.aNrmNow_hist[period - 1, :]
                * base_params["Rfree"]
                / CheckType.PermShkNow_hist[period]
                + CheckType.TranShkNow_hist[period]
            )
        else:
            m_adj = (
                np.exp(base_params["aNrmInitMean"])
                * base_params["Rfree"]
                / CheckType.PermShkNow_hist[period]
                + CheckType.TranShkNow_hist[period]
            )
        ...
        m_adj = (
            a[period - 1, :] * base_params["Rfree"] / CheckType.PermShkNow_hist[period]
            + CheckType.TranShkNow_hist[period]
        )
...
print(
    "TransShokNow is exactly 1 for indices",
    np.where(CheckType.TranShkNow_hist == 1),
    "\n",
)
