...
from HARK.utilities import approxUniform, getLorenzShares, getPercentiles
...
[
    ...
    LotteryWin,
    Lorenz_Data,
] = FagerengObjFunc(opt[0], opt[1], opt[2], estimation_mode=False, target="AGG_MPC")
print(
    "Agg MPC from first year to year t+4 \n", simulated_MPC_mean_add_Lottery_Bin, "\n"
)  # %% Plot aggregate MPC and MPCX
...
def Outer_Loop(splurge):
    ...
    dist = FagerengObjFunc(
        splurge,
        current_beta_nabla[0],
        current_beta_nabla[1],
        target="MPC_plus_AGG_MPC_1",
    )
...
def Inner_Loop(splurge):
    ...
    opt_current_beta_nabla = minimizeNelderMead(
        f_inner_temp, current_beta_nabla, xtol=1e-3, ftol=1e-3, verbose=True
    )
    current_beta_nabla = opt_current_beta_nabla
    print(
        "Current optimal (beta,nabla), for splurge factor = ",
        splurge,
        "  is " + str(current_beta_nabla[0]) + "," + str(current_beta_nabla[1]) + "\n",
    )


f_outer_temp = lambda x: Outer_Loop(x)
opt = minimizeNelderMead(
    f_outer_temp, guess_splurge_factor, verbose=True, xtol=1e-3, ftol=1e-3
)
...
[
    ...
    opt, current_beta_nabla[0], current_beta_nabla[1], estimation_mode=False
)
print("Distance for Agg MPC is", distance_Agg_MPC, "\n")
print("Distance for MPC matrix is", distance_MPC, "\n")
