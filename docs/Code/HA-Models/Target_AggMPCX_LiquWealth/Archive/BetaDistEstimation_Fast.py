...
base_params[
    "T_age"
] = 400  # Kill off agents if they manage to achieve T_kill working years
...
if Parametrization == "NOR_Growth15":
    ...
    base_params["TranShkStd"] = [(0.03 * 4) ** 0.5]
    base_params["BoroCnstArt"] = -0.8
    base_params["PermGroFac"] = [1.015**0.25]
...
lottery_size_NOK = lottery_size_USD * (
    10 / 1.1
)  # in Fagereng et al it is mention that 1000 NOK = 110 USD
lottery_size = lottery_size_NOK / (270 / 4)
# divide by permanent income.
...
def FagerengObjFunc(
    ...
    multiThreadCommands(
        EstTypeList, ["solve()", "initializeSim()", "simulate()", "unpackcFunc()"]
    )
    ...
    Lorenz_Data = getLorenzShares(
        WealthNow_sorted, percentiles=np.arange(0.01, 1.00, 0.01), presorted=True
    )
    ...
    Wealth_Perm_Ratio_adj = (
        Wealth_Perm_Ratio - Wealth_Perm_Ratio[0]
    )  # add lowest possible value to everyone
    Lorenz_Data_Adj = getLorenzShares(
        Wealth_Perm_Ratio_adj, percentiles=np.arange(0.01, 1.00, 0.01), presorted=True
    )
    Lorenz_Data_Adj = np.hstack((np.array(0.0), Lorenz_Data_Adj, np.array(1.0)))
    lorenz_Model = np.array(
        [
            Lorenz_Data_Adj[20],
            Lorenz_Data_Adj[40],
            Lorenz_Data_Adj[60],
    ...
    ThisType.EPermShkInv = np.dot(
        ThisType.InvPermShkDstn[0].pmf, 1 / ThisType.PermShkDstn[0].X
    )
    ThisType.InvEPermShkInv = 1 / ThisType.EPermShkInv
    ThisType.thorn = (ThisType.Rfree * ThisType.DiscFac * ThisType.LivPrb[0]) ** (
        1 / ThisType.CRRA
    )
    ThisType.GPFInd = ThisType.thorn / (
        ThisType.PermGroFac[0] * ThisType.InvEPermShkInv
    )  # [url]/#GICI
    # ThisType.GPFInd = EstTypeList[7].GPFInd - 0.0025 #manual adj. for max age
    # ThisType.GPFInd = EstTypeList[7].GPFInd - 1 # switch off constrint
    print("EstTypeList[7].GPFInd = ", EstTypeList[7].GPFInd)

    N_Quarter_Sim = 20
    # Needs to be dividable by four
    ...
    for ThisType in EstTypeList:
        c_base = np.zeros(
            (ThisType.AgentCount, N_Quarter_Sim)
        )  # c_base (in case of no lottery win) for each quarter
        c_base_Lvl = np.zeros((ThisType.AgentCount, N_Quarter_Sim))  # same in levels
        c_actu = np.zeros(
            (ThisType.AgentCount, N_Quarter_Sim, N_Lottery_Win_Sizes)
        )  # c_actu (actual consumption in case of lottery win in one random quarter) for each quarter and lottery size
        c_actu_Lvl = np.zeros(
            (ThisType.AgentCount, N_Quarter_Sim, N_Lottery_Win_Sizes)
        )  # same in levels
        a_actu = np.zeros(
            (ThisType.AgentCount, N_Quarter_Sim, N_Lottery_Win_Sizes)
        )  # a_actu captures the actual market resources after potential lottery win (last index) was added and c_actu deducted
        ...
        MPC_this_type = np.zeros(
            (ThisType.AgentCount, N_Lottery_Win_Sizes, N_Year_Sim)
        )  # Empty array, MPC for each Lottery size and agent

        for period in range(
            N_Quarter_Sim
        ):  # Simulate for 4 quarters as opposed to 1 year
            ...
            k = 4
            # do not loop to save time

            Llvl = (
                lottery_size[k] * LotteryWin[:, period]
            )  # Lottery win occurs only if LotteryWin = 1 for that agent
            ...
            SplurgeNrm = (
                SplurgeEstimate * Lnrm
            )  # Splurge occurs only if LotteryWin = 1 for that agent
            ...
            if period == 0:
                ...
                a_actu[:, period, k] = (
                    ThisType.mNrmNow + Lnrm - c_actu[:, period, k]
                )  # save for next periods
            else:
                ...
                for i_agent in range(ThisType.AgentCount):
                    if ThisType.TranShkNow[i_agent] == 1.0:  # indicator of death
                        a_actu[i_agent, period - 1, k] = np.exp(
                            base_params["aNrmInitMean"]
                        )
                m_adj = (
                    a_actu[:, period - 1, k] * R_kink / ThisType.PermShkNow
                    + ThisType.TranShkNow
                    + Lnrm
                    - SplurgeNrm
                )  # continue with resources from last period
                c_actu[:, period, k] = ThisType.cFunc[0](m_adj) + SplurgeNrm
                c_actu_Lvl[:, period, k] = c_actu[:, period, k] * ThisType.pLvlNow
                a_actu[:, period, k] = (
                    a_actu[:, period - 1, k] * R_kink / ThisType.PermShkNow
            ...
            if period % 4 + 1 == 4:  # if we are in the 4th quarter of a year
                ...
                MPC_this_type[:, k, year - 1] = (
                    np.sum(c_actu_Lvl_year, axis=1) - np.sum(c_base_Lvl_year, axis=1)
                ) / (lottery_size[k])
        ...
        if estimation_mode == False:
            ...
            for q in range(4):
                ...
                for k in range(
                    N_Lottery_Win_Sizes - 1
                ):  # only consider here 4 Lottery bins
...
splurge = 0.32883075091278136
beta = 0.985556105486823
nabla = 0.018261886521488274

splurge = 0.3066459443821208
beta = 0.9826045188577284
nabla = 0.017937805628964937
...
[
    ...
    splurge, beta, nabla, estimation_mode=False, target="AGG_MPC_plus_Liqu_Wealth"
)

print("Results for parametrization: ", Parametrization)
print(
    "Agg MPC from first year to year t+4 \n", simulated_MPC_mean_add_Lottery_Bin
...
xAxis = np.arange(0, 5)
(line1,) = plt.plot(
    xAxis, simulated_MPC_mean_add_Lottery_Bin, ":b", linewidth=2, label="Model"
)
...
print(
    "Model: Lorenz shares at 20th, 40th, 60th and 80th percentile",
    Lorenz_Data_Adj[20],
    Lorenz_Data_Adj[40],
    Lorenz_Data_Adj[60],
    Lorenz_Data_Adj[80],
...
print(
    "Percentile with zero cummulative assets", np.argwhere(Lorenz_Data > 0)[0] - 1, "%"
)

LorenzAxis = np.arange(101, dtype=float)
lorenz_target_interp = np.interp(
    LorenzAxis, np.array([20, 40, 60, 80, 100]), np.hstack([lorenz_target, 1])
)
...
def z_function(x, y, fixed_splurge):
    [
        ...
        fixed_splurge, x, y, estimation_mode=False, target="AGG_MPC_plus_Liqu_Wealth"
    )
    return distance
...
def f_beta(splurge, beta, nabla):
    ...
    f_nabla = lambda x: FagerengObjFunc(
        splurge, beta, x, target="AGG_MPC_plus_Liqu_Wealth"
    )
    opt_nabla = minimizeNelderMead(
        f_nabla, np.array([start_nabla]), verbose=True, xtol=1e-3, ftol=1e-3
    )
    print(
        "Found optimal nabla = ",
        opt_nabla,
    ...
    distance = FagerengObjFunc(
        splurge, beta, opt_nabla, target="AGG_MPC_plus_Liqu_Wealth"
    )
    return distance


f_temp = lambda x: f_beta(splurge, x, start_nabla)
opt_beta = minimizeNelderMead(
    f_temp, np.array([start_beta]), verbose=True, xtol=1e-3, ftol=1e-3
)
...
def f_splurge(splurge, beta, nabla):
    ...
    f_beta_nabla = lambda x: FagerengObjFunc(
        splurge, x[0], x[1], target="AGG_MPC_plus_Liqu_Wealth"
    )
...
def z_function(x, y, z):
    [
        ...
        z, x, y, estimation_mode=False, target="AGG_MPC_plus_Liqu_Wealth"
    )
    return distance
