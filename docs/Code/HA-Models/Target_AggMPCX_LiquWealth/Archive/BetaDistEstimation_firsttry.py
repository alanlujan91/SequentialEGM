...
def FagerengObjFunc(SplurgeEstimate, center, spread, verbose=False):
    ...
    for ThisType in EstTypeList:
        ...
        c_actu = np.zeros(
            (ThisType.AgentCount, N_Quarter_Sim, 4)
        ...
        MPC_this_type = np.zeros(
            (ThisType.AgentCount, 4, int(N_Quarter_Sim / 4))
        ...
        for period in range(
            ...
            for k in range(4):  # Loop through different lottery sizes
                ...
                SplurgeNrm = (
                    SplurgeEstimate / ThisType.pLvlNow * LotteryWin[:, period]
...
[
    ...
    T_hist,
    P_hist,
] = FagerengObjFunc(Splurge, beta_dist_estimate[0], beta_dist_estimate[1])
