...
from HARK.utilities import getLorenzShares, getPercentiles, make_figs
...
def FagerengObjFunc(
    ...
    ThisType.GPFInd = ThisType.thorn / (
        ThisType.PermGroFacAgg * ThisType.PermGroFac[0] * ThisType.InvEPermShkInv
    )  # [url]/#GICI
...
def find_Opt_splurge_beta_nabla():
    ...
    f_temp = lambda x: FagerengObjFunc(
        x[0], x[1], x[2], target="AGG_MPC_plus_Liqu_Wealth"
    )
    opt = minimizeNelderMead(
        f_temp, guess_splurge_beta_nabla, verbose=True, maxiter=100
    )
...
for el in range(0, len(CRRA_values)):
    ...
    with open(
        Abs_Path + "/Result_CRRA_" + str(base_params["CRRA"]) + ".0.txt", "w"
    ) as f:
        str1 = repr(res)
        f.write(str1)
        f.close
...
f = open(Abs_Path + r"\Result_CRRA_2.0.txt")
if f.mode == "r":
    contents = f.read()
