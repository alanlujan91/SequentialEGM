...
import numpy as np
from HARK.ConsumptionSaving.ConsIndShockModel import ConsumerSolution, MargValueFunc
from HARK.ConsumptionSaving.ConsMarkovModel import MarkovConsumerType
from HARK.core import distanceMetric
from HARK.distribution import Bernoulli, DiscreteDistribution, Uniform
from HARK.interpolation import LinearInterp, LowerEnvelope
from Parameters import T_sim, makeCondMrkvArrays, makeFullMrkvArray, makeMacroMrkvArray
...
class FiscalType(MarkovConsumerType):
    ...
    def getStates(self):
        ...
        self.MrkvNowPcvd = np.remainder(self.MrkvNow, 3) + 3 * np.floor_divide(
            self.MrkvNowPcvd, 3
        )
    ...
    def getMacroMarkovStates(self):
        ...
        other_transitions = self.MacroMrkvArray[np.array(range(int(J / 2))) * 2, :][
            :, np.array(range(int(J / 2))) * 2
        ]
        recession_draws = Uniform(seed=self.RNG.randint(0, 2**31 - 1)).draw(
            self.AgentCount
        )
        other_draws = Uniform(seed=self.RNG.randint(0, 2**31 - 1)).draw(
            self.AgentCount
        )
        ...
        for j in range(J):
            Recession = np.searchsorted(
                RecessionCutoffs[j % 2, :], recession_draws[MacroMrkvBoolArray[j, :]]
            ).astype(int)
            Other = np.searchsorted(
                OtherCutoffs[np.floor(j / 2).astype(int), :],
                other_draws[MacroMrkvBoolArray[j, :]],
            ).astype(int)
    ...
    def updateMrkvArray(self):
        MacroMrkvArray = makeMacroMrkvArray(
            self.Rspell,
            self.PolicyUBspell,
            self.TaxCutPeriods,
            self.TaxCutContinuationProb_Rec,
            self.TaxCutContinuationProb_Bas,
        )
        CondMrkvArrays = makeCondMrkvArrays(
            self.Urate_normal,
            self.Uspell_normal,
            self.UBspell_normal,
            self.Urate_recession,
            self.Uspell_recession,
            self.UBspell_extended,
            self.TaxCutPeriods,
        )
    ...
    def hitWithRecessionShock(self):
        ...
        ] = 2  # Move people from employment to unemployment such that total unemployment rate is as required. Don't touch already unemployed people.

        # $$$$$$$$$$
        if (
            self.RecessionShock and not self.R_shared
        ...
        tax_cut_multiplier[
            np.greater(self.history["MrkvNow"], 11)
        ] *= (
            self.TaxCutIncFactor
        )  # $$$$$$$$$$ assumes all markov states above 11 are tax cut states
...
def solveConsMarkovALT(
    ...
    for j in range(StateCount):
        ...
        PermShkValsNext_tiled = np.tile(
            np.reshape(PermShkValsNext, (1, ShkCount)), (aCount, 1)
        )
        TranShkValsNext_tiled = np.tile(
            np.reshape(TranShkValsNext, (1, ShkCount)), (aCount, 1)
        )
        ...
        aNrmMin_candidates = (
            ...
            * PermShkValsNext_tiled
            / Rfree[j]
            * (mNrmMinNext - TranShkValsNext_tiled[0, :])
        )
        ...
        vPnext_array = (
            Rfree[j] * PermShkValsNext_tiled ** (-CRRA) * vPfuncNext(mNrmNext_array)
        )
        ...
        EndOfPrdvPnvrs = EndOfPrdvP ** (-1.0 / CRRA)
        EndOfPrdvPnvrsFunc = LinearInterp(
            np.insert(aNrmMin + aXtraGrid, 0, aNrmMin),
            np.insert(EndOfPrdvPnvrs, 0, 0.0),
        )
    ...
    for i in range(StateCount):
        ...
        uP = c_temp ** (-CRRA)
        vPnvrs = uP ** (-1.0 / CRRA)
        vPnvrsFunc = LinearInterp(
            np.insert(m_temp, 0, mNrmMin), np.insert(vPnvrs, 0, 0.0)
        )
