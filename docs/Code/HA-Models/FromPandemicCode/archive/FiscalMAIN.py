...
if __name__ == "__main__":
    ...
    IncomeDstn_unemp = DiscreteDistribution(
        np.array([1.0]), [np.array([1.0]), np.array([InfHorizonType.IncUnemp])]
    )
    IncomeDstn_unemp_nobenefits = DiscreteDistribution(
        np.array([1.0]),
        [np.array([1.0]), np.array([InfHorizonType.IncUnempNoBenefits])],
    )
    IncomeDstn_big = []
    for ThisType in BaseTypeList:
        ...
        IncomeDstn_big.append(
            [
                ...
                IncomeDstn_unemp,  # normal
                ThisType.IncomeDstn[0],
                IncomeDstn_unemp_nobenefits,
                IncomeDstn_unemp,  # recession
                ThisType.IncomeDstn[0],
                IncomeDstn_unemp_nobenefits,
                IncomeDstn_unemp,  # normal, extended UI
                ThisType.IncomeDstn[0],
                IncomeDstn_unemp_nobenefits,
                IncomeDstn_unemp,  # recession, extended UI
                IncomeDstn_taxcut,
                IncomeDstn_unemp_nobenefits,
                IncomeDstn_unemp,  # normal, payroll tax cut
                IncomeDstn_taxcut,
                IncomeDstn_unemp_nobenefits,
                IncomeDstn_unemp,  # recession, payroll tax cut
                IncomeDstn_taxcut,
                IncomeDstn_unemp_nobenefits,
                IncomeDstn_unemp,  # normal, payroll tax cut
                IncomeDstn_taxcut,
        ...
        ThisType.IncomeDstn_big = (
            IncomeDstn_big  # Comment income distributions for each state
        )
    ...
    for b in range(DiscFacDstns[0].X.size):
        for e in range(num_types):
            ...
            AgentCount = int(
                np.floor(AgentCountTotal * TypeShares[e] * DiscFacDstns[e].pmf[b])
            )
    ...
    baseline_commands = [
        ...
        "simulate()",
        "saveState()",
        "switchToCounterfactualMode()",
        "makeAlternateShockHistories()",
    ]
    multiThreadCommandsFake(TypeList, baseline_commands)
    t1 = time()
    print(
        "Making the baseline distribution of states and preparing to run counterfactual simulations took "
        + mystr(t1 - t0)
        + " seconds."
    )
    ...
    t1 = time()
    print(
        "Calculating payroll tax cut consumption took " + mystr(t1 - t0) + " seconds."
    )
    ...
    t1 = time()
    print(
        "Calculating recession and extended UI consumption took "
        + mystr(t1 - t0)
        + " seconds."
    )
    ...
    t1 = time()
    print(
        "Calculating recession and payroll tax cut consumption took "
        + mystr(t1 - t0)
        + " seconds."
    )
    ...
    plt.legend(
        [
            "Fiscal policy expenditure, UI extension",
            "Additional consumption, UI extension",
        ]
    )
    plt.savefig(figs_dir + "UI_cut" + add_plot_text + ".pdf")
    plt.show()
    Stimulus_UI = (
        AddCons / NPV_AddInc[-1]
    )  # divide by total cumulative NPV of the policy
    ...
    plt.legend(
        [
            "Fiscal policy expenditure, UI extension during recession",
            "Additional consumption, UI extension during recession",
        ]
    )
    ...
    plt.plot(AddCons)
    plt.legend(
        ["Fiscal policy expenditure, tax cut", "Additional consumption, tax cut"]
    )
    ...
    plt.legend(
        [
            "Fiscal policy expenditure, tax cut during recession",
            "Additional consumption, tax cut during recession",
        ]
    )
    ...
    plt.title(
        "Stimulated consumption per period relative to NPV of policy intervention"
    )
