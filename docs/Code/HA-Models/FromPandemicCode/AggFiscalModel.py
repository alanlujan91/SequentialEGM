...
from HARK.core import HARKobject, distanceMetric
...
[
    makeMacroMrkvArray_recession,
    makeCondMrkvArrays_recession,
    makeFullMrkvArray,
    T_sim,
    makeCondMrkvArrays_base,
    makeCondMrkvArrays_recessionUI,
] = returnParameters(OutputFor="_Model.py")
...
class AggFiscalType(MarkovConsumerType):
    ...
    def initializeSim(self):
        ...
        self.MacroMrkvNow = (np.floor(self.MrkvNow / self.num_base_MrkvStates)).astype(
            int
        )
    ...
    def getEconomyData(self, Economy):
        ...
        self.Cgrid = (
            Economy.CgridBase
        )  # Ratio of consumption to steady state consumption
        self.CFunc = Economy.CFunc  # Next period's consumption ratio function
        self.ADFunc = (
            Economy.ADFunc
        )  # Function that takes aggregate consumption to agg. demand function
        self.addToTimeInv(
            "Cgrid", "CFunc", "ADFunc", "num_experiment_periods", "num_base_MrkvStates"
        )
    ...
    def hitWithRecessionShock(self, shock_type):
        ...
        if (
            ...
            or shock_type == "recessionUI"
            or shock_type == "recessionTaxCut"
            or shock_type == "recessionCheck"
        ):
            this_Urate = self.Urate_recession
        elif (
            shock_type == "base"
            or shock_type == "UI"
            or shock_type == "TaxCut"
            or shock_type == "Check"
        ):
        ...
        MrkvNew[
            np.logical_and(np.equal(self.MrkvNow, 0), draws_empy2umemp)
        ] = 1  # Move people from employment to unemployment such that total unemployment rate is as required. Don't touch already unemployed people.
        ...
        if shock_type == "base":
            ...
            or shock_type == "recessionTaxCut"
            or shock_type == "recessionCheck"
        ):  # If the recssion actually occurs,
            MrkvNew += (
                3 * self.num_base_MrkvStates
            )  # then put everyone into the recession ???????????????????????
        elif (
            shock_type == shock_type == "UI"
            or shock_type == "TaxCut"
            or shock_type == "Check"
        ):
            MrkvNew += (
                2 * self.num_base_MrkvStates
            )  # then put everyone into first experiment mrkv state ???????????????????????
        ...
        for t in range(self.T_sim):
            self.t_age = (
                1 - self.who_dies_fixed_hist[t]
            )  # hack to get newborns have t_age=0
        ...
        if shock_type == "recessionTaxCut" or shock_type == "TaxCut":
            tax_cut_states = np.logical_and(
                np.greater(self.history["MrkvNow"], 2 * self.num_base_MrkvStates - 1),
                np.less(self.history["MrkvNow"], 9 * 2 * self.num_base_MrkvStates),
            )  # assumes tax cut last 8 periods
            ...
            for agent in range(len(CheckAmount[0])):
                ...
                if self.pLvlNow[agent] < self.CheckStimLvl_PLvl_Cutoff_start:
                    ...
                    AgentSpecificScalar = 1 - (
                        self.pLvlNow[agent] - self.CheckStimLvl_PLvl_Cutoff_start
                    ) / (
                        self.CheckStimLvl_PLvl_Cutoff_end
                        - self.CheckStimLvl_PLvl_Cutoff_start
                    )
        ...
        self.history["TranShkNow"][employed] = (
            self.tran_shock_fixed_hist[employed] * tax_cut_multiplier[employed]
            + CheckAmount[employed] / self.perm_shock_fixed_hist[employed]
        )
        unemp_without_benefits = np.equal(
            self.history["MrkvNow"] % self.num_base_MrkvStates,
            self.num_base_MrkvStates - 1,
        )
        self.history["PermShkNow"][unemp_without_benefits] = 1.0
        self.history["TranShkNow"][unemp_without_benefits] = (
            self.IncUnempNoBenefits + CheckAmount[unemp_without_benefits]
        )
        unemp_with_benefits = np.logical_not(
            np.logical_or(employed, unemp_without_benefits)
        )
        self.history["PermShkNow"][unemp_with_benefits] = 1.0
        self.history["TranShkNow"][unemp_with_benefits] = (
            self.IncUnemp + CheckAmount[unemp_with_benefits]
        )
    ...
    def getShocks(self):
        ...
        self.update_draw = self.RNG.permutation(
            np.array(range(self.AgentCount))
        )  # A list permuted integers, low draws will update their aggregate Markov state
        if hasattr(self, "Mrkv_univ") and self.Mrkv_univ is not None:
            self.MrkvNow = self.MrkvNow_temp  # Make sure real sequence is recorded
        self.update_draw = self.RNG.permutation(
            np.array(range(self.AgentCount))
    ...
    def getStates(self):
        ...
        self.MrkvNowPcvd = np.remainder(
            self.MrkvNow, self.num_base_MrkvStates
        ) + self.num_base_MrkvStates * np.floor_divide(
            self.MrkvNowPcvd, self.num_base_MrkvStates
        )
        self.mNrmNow = (
            self.bNrmNow + self.TranShkNow * self.AggDemandFac
        )  # Market resources after income accounting for Agg Demand factor (this is for simulation)
    ...
    def getMicroMarkvStates_guts(self, unemployment_draw):
        dont_change = (
            self.t_age == 0
        )  # Don't change Markov state for those who were just born
    ...
    def updateMrkvArray(self, shock_type):
        if shock_type == "base":
            ...
            self.CondMrkvArrays = makeCondMrkvArrays_base(
                self.Urate_normal, self.Uspell_normal, self.UBspell_normal
            )
            self.MrkvArray = makeFullMrkvArray(self.MacroMrkvArray, self.CondMrkvArrays)
        elif shock_type == "recession":
            self.MacroMrkvArray = makeMacroMrkvArray_recession(
                self.Rspell, self.num_experiment_periods
            )
            self.CondMrkvArrays = makeCondMrkvArrays_recession(
                self.Urate_normal,
                self.Uspell_normal,
                self.UBspell_normal,
                self.Urate_recession,
                self.Uspell_recession,
                self.num_experiment_periods,
            )
            self.MrkvArray = makeFullMrkvArray(self.MacroMrkvArray, self.CondMrkvArrays)
        elif shock_type == "recessionUI" or shock_type == "UI":
            self.MacroMrkvArray = makeMacroMrkvArray_recession(
            ...
            self.CondMrkvArrays = makeCondMrkvArrays_recessionUI(
                self.Urate_normal,
                self.Uspell_normal,
                self.UBspell_normal,
                self.Urate_recession,
                self.Uspell_recession,
                self.num_experiment_periods,
                self.UBspell_extended - self.UBspell_normal,
            )
            self.MrkvArray = makeFullMrkvArray(self.MacroMrkvArray, self.CondMrkvArrays)
        elif shock_type == "recessionTaxCut" or shock_type == "TaxCut":
            self.MacroMrkvArray = makeMacroMrkvArray_recession(
    ...
    def solveIfChanged(self):
        ...
        try:
            same_MrkvArray = distanceMetric(self.MrkvArray, self.MrkvArray_prev) == 0.0
            if same_MrkvArray:
                return
        except:
            pass
    ...
    def calcAgeDistribution(self):
        ...
        self.LRageDstn = DiscreteDistribution(
            LRagePrbs, age_vec, seed=self.RNG.randint(0, 2**31 - 1)
        )
    ...
    def getControls(self):
        ...
        for j in range(J):
            MrkvBoolArray[j, :] = (
                j == self.MrkvNowPcvd
            )  # agents choose control based on *perceived* Markov state
        ...
        for t in range(self.T_cycle):
            ...
            for j in range(J):
                ...
                cNrmNow[these] = self.solution[t].cFunc[j](
                    self.mNrmNow[these], CratioNow[these]
                )
                # Marginal propensity to consume
                MPCnow[these] = (
                    self.solution[t]
                    .cFunc[j]
                    .derivativeX(self.mNrmNow[these], CratioNow[these])
                )
        ...
        self.cLvl_splurgeNow = (
            (1.0 - self.Splurge) * self.cLvlNow
            + self.Splurge * self.pLvlNow * self.TranShkNow * self.AggDemandFac
        )  # added last term relaive to Edmund's Version

    def reset(self):
        return  # do nothing


def solveAggConsMarkovALT(
    solution_next,
    IncomeDstn,
    LivPrb,
    DiscFac,
    ...
    for j in range(StateCount):
        ...
        aXtra_tiled = np.tile(
            np.reshape(aXtraGrid, (1, aCount, 1)), (Ccount, 1, ShkCount)
        )

        # Make tiled versions of the income shocks
        # Dimension order: aNow, Shk
        ShkPrbsNext_tiled = np.tile(
            np.reshape(ShkPrbsNext, (1, 1, ShkCount)), (Ccount, aCount, 1)
        )
        PermShkValsNext_tiled = np.tile(
            np.reshape(PermShkValsNext, (1, 1, ShkCount)), (Ccount, aCount, 1)
        )
        TranShkValsNext_tiled_noAD = np.tile(
            np.reshape(TranShkValsNext, (1, 1, ShkCount)), (Ccount, aCount, 1)
        )
        ...
        aNrmMin_tiled = np.tile(
            np.reshape(aNrmMin_vec, (Ccount, 1, 1)), (1, aCount, ShkCount)
        )
        ...
        mNrmNext_array = (
            Rfree[j] * aNrmNow_tiled / (PermGroFac[j] * PermShkValsNext_tiled)
            + TranShkValsNext_tiled
        )

        # Find marginal value next period at every income shock realization and every aggregate market resource gridpoint
        vPnext_array = (
            Rfree[j]
            * PermShkValsNext_tiled ** (-CRRA)
            * vPfuncNext(mNrmNext_array, Cnext_array)
        )
        ...
        EndOfPrdvPnvrs = np.concatenate(
            (np.zeros((Ccount, 1)), EndOfPrdvP ** (-1.0 / CRRA)), axis=1
        )
        EndOfPrdvPnvrsFunc_base = BilinearInterp(
            np.transpose(EndOfPrdvPnvrs), np.insert(aXtraGrid, 0, 0.0), Cgrid
        )
...
class AggregateDemandEconomy(Market):
    ...
    def __init__(self, agents=None, **kwds):
        """
    ...
    def millRule(self, cLvl_splurgeNow):
        ...
        if hasattr(self, "base_AggCons"):
            cLvl_all_splurge = np.concatenate(
                [this_cLvl for this_cLvl in cLvl_splurgeNow]
            )
            AggCons = np.sum(cLvl_all_splurge)
            self.CratioNow = AggCons / self.base_AggCons[self.Shk_idx]
            CratioNext = self.CFunc[EconomyMrkvNow * self.num_base_MrkvStates][
                EconomyMrkvNext * self.num_base_MrkvStates
            ](self.CratioNow)
        else:
            self.CratioNow = 1.0
            CratioNext = 1.0
    ...
    def runExperiment(
        ...
        self.CratioNow_init = self.CFunc[0][
            EconomyMrkv_init[0] * self.num_base_MrkvStates
        ].intercept
        ...
        cNrm_all = np.concatenate(
            [ThisType.history["cNrmNow"] for ThisType in self.agents], axis=1
        )
        Mrkv_hist = np.concatenate(
            [ThisType.history["MrkvNow"] for ThisType in self.agents], axis=1
        )
        pLvl_all = np.concatenate(
            [ThisType.history["pLvlNow"] for ThisType in self.agents], axis=1
        )
        TranShk_all = np.concatenate(
            [ThisType.history["TranShkNow"] for ThisType in self.agents], axis=1
        )
        mNrm_all = np.concatenate(
            [ThisType.history["mNrmNow"] for ThisType in self.agents], axis=1
        )
        aNrm_all = np.concatenate(
            [ThisType.history["aNrmNow"] for ThisType in self.agents], axis=1
        )
        cLvl_all = np.concatenate(
            [ThisType.history["cLvlNow"] for ThisType in self.agents], axis=1
        )
        cLvl_all_splurge = np.concatenate(
            [ThisType.history["cLvl_splurgeNow"] for ThisType in self.agents], axis=1
        )

        IndIncome = (
            pLvl_all * TranShk_all * np.array(self.history["AggDemandFacPrev"])[:, None]
        )
        ...
        Mrkv_init = np.concatenate(
            [ThisType.history["MrkvNow"][0, :] for ThisType in self.agents]
        )
    ...
    def storeADsolution(self, name):
        ...
        for i in range(len(self.agents)):
            self.stored_solutions[name].agent_solutions.append(
                copy(self.agents[i].solution)
            )
    ...
    def CompareCFuncConvergence(self, Old_Cfunc, New_Cfunc):
        ...
        for i in range(dim):
            for j in range(dim):
                ...
                DiffIntercepts[i, j] = abs(
                    New_Cfunc[i][j].intercept - Old_Cfunc[i][j].intercept
                )
        ...
        FromMrkState_Intercept_Largest_Diff = int(
            np.floor(i / self.num_base_MrkvStates)
        )
    ...
    def solveAD_Recession(
        ...
        for i in range(num_max_iterations):
            print("Iteration ", i + 1, ":")
            recession_dict["EconomyMrkv_init"] = (
                list(np.arange(1, self.num_experiment_periods + 1) * 2 + 1)
                + [1] * 12
                + [0] * 20
            )
            ...
            for j in range(self.num_experiment_periods - 1):
                MacroCFunc[2 * j + 3][2 * j + 5] = CRule(
                    recession_results["Cratio_hist"][j + 1], 0.0
                )
            ...
            MacroCFunc[1][1] = CRule(
                np.mean(
                    recession_results["Cratio_hist"][
                        self.num_experiment_periods
                        + 1 : self.num_experiment_periods
                        + 10
                    ]
                ),
                0.0,
            ...
            for ii in range(dim):
                for jj in range(dim):
                    Step_Cfunc[ii][jj].slope = Old_Cfunc[ii][jj].slope + step * (
                        New_Cfunc[ii][jj].slope - Old_Cfunc[ii][jj].slope
                    )
                    Step_Cfunc[ii][jj].intercept = Old_Cfunc[ii][
                        jj
                    ].intercept + step * (
                        New_Cfunc[ii][jj].intercept - Old_Cfunc[ii][jj].intercept
                    )
    ...
    def solveAD_Check_Recession(
        self, num_max_iterations, convergence_cutoff=1e-3, name=None
    ):
        self.solveAD_Recession(
            num_max_iterations,
            convergence_cutoff,
            name=name,
    ...
    def solveAD_UIExtension_Recession(
        self, num_max_iterations, convergence_cutoff=1e-3, name=None
    ):
        self.solveAD_Recession(
            num_max_iterations, convergence_cutoff, name=name, shock_type="recessionUI"
        )

    def solveAD_Recession_TaxCut(
        self, num_max_iterations, convergence_cutoff=1e-3, name=None
    ):
        self.solveAD_Recession(
            num_max_iterations,
