...
reload(
    ep
)  # Force reload in case the code is running from commandline for different values
...
def calcEstimStats(Agents):
    ...
    # Lorenz points:
    LorenzPts = 100 * getLorenzShares(
        aLvlAll, weights=weights, percentiles=[0.2, 0.4, 0.6, 0.8]
    )
    ...
    for e in range(num_types):
        aNrmAll_byEd = []
        aNrmAll_byEd = np.concatenate(
            [
                ThisType.aNrmNow
                for ThisType in Agents[e * DiscFacCount : (e + 1) * DiscFacCount]
            ]
        )
        ...
        medianLWPI[e] = 100 * getPercentiles(
            aNrmAll_byEd, weights=weights, percentiles=[0.5]
        )
...
# -----------------------------------------------------------------------------
def checkDiscFacDistribution(
    beta,
    nabla,
    educ_type,
    print_mode=False,
    print_file=False,
    filename="DefaultResultsFile.txt",
):
    """
    ...
    if print_mode:
        print(
            "Approximation to beta distribution: betaMin = "
            + str(round(betaMin, 5))
            + ", betaMax = "
            + str(round(betaMax, 5))
            + "\n"
        ...
        print(
            "Imposed GIC consistent maximum beta = "
            + str(round(GICmaxBetas[educ_type] * GICfactor, 5))
            + "\n\n"
        )
...
for e in range(num_types):
    for b in range(DiscFacCount):
        ...
        AgentCount = int(
            np.floor(AgentCountTotal * data_EducShares[e] * DiscFacDstns[e].pmf[b])
        )
...
def betasObjFunc(
    ...
    for e in range(num_types):
        for b in range(DiscFacCount):
            AgentCount = int(
                np.floor(AgentCountTotal * data_EducShares[e] * dfs[e].pmf[b])
            )
            ThisType = deepcopy(BaseTypeList[e])
            ThisType.AgentCount = AgentCount
    ...
    if print_mode:
        ...
        print(
            "Median LW/PI-ratios: D = "
            + mystr(Stats.medianLWPI[0][0])
            + " H = "
            + mystr(Stats.medianLWPI[1][0])
            + " C = "
            + mystr(Stats.medianLWPI[2][0])
        )
        ...
        print(
            "Average LW/PI-ratios: D = "
            + mystr(Stats.avgLWPI[0])
            + " H = "
            + mystr(Stats.avgLWPI[1])
            + " C = "
            + mystr(Stats.avgLWPI[2])
        )
        print(
            "Total LW/Total PI: D = "
            + mystr(Stats.LWoPI[0])
        ...
        print(
            "Wealth Shares: D = "
            + mystr(WealthShares[0])
            + " H = "
            + mystr(WealthShares[1])
            + " C = "
            + mystr(WealthShares[2])
        )
        print(
            "Average MPCs (incl. splurge) = ["
            + str(round(MPCs.MPCsA[0], 3))
            + ", "
            + str(round(MPCs.MPCsA[1], 3))
            + ", "
            + str(round(MPCs.MPCsA[2], 3))
            + ", "
    ...
    if print_file:
        with open(filename, "a") as resFile:
            ...
            resFile.write(
                "\tLorenz Points = ["
                + str(round(Stats.LorenzPts[0], 4))
                + ", "
                + str(round(Stats.LorenzPts[1], 4))
                + ", "
                + str(round(Stats.LorenzPts[2], 4))
                + ", "
            ...
            resFile.write(
                "\tWealth shares = ["
                + str(round(WealthShares[0], 3))
                + ", "
                + str(round(WealthShares[1], 3))
                + ", "
                + str(round(WealthShares[2], 3))
                + "]\n"
            )
            resFile.write(
                "\tAverage MPCs (incl. splurge) = ["
...
def betasObjFuncEduc(
    ...
    for b in range(DiscFacCount):
        AgentCount = int(
            np.floor(AgentCountTotal * data_EducShares[educ_type] * dfs.pmf[b])
        )
    ...
    TypeListAll[
        educ_type * DiscFacCount : (educ_type + 1) * DiscFacCount
    ] = TypeListNewEduc
    ...
    if print_mode:
        print(
            "Median LW/PI-ratio for group e = "
            + mystr(educ_type)
            + " is: "
        ...
        print(
            "Average LW/PI-ratios for group e = "
            + mystr(educ_type)
            + " is: "
    ...
    if print_file:
        with open(filename, "a") as resFile:
            ...
            resFile.write(
                "\tMedian LW/PI-ratio = " + mystr(Stats.medianLWPI[educ_type][0]) + "\n"
            )
            resFile.write(
                "\tLorenz Points = ["
                + str(round(lp[0], 4))
...
if IncUnemp == 0.7 and IncUnempNoBenefits == 0.5:
    ...
    print(
        "Estimating for an alternativ unemployment system with IncUnemp = "
        + str(round(IncUnemp, 2))
        + " and IncUnempNoBenefits = "
        + str(round(IncUnempNoBenefits, 2))
        + ":\n"
    )
    df_resFileStr = (
        res_dir
...
for edType in [0, 1, 2]:
    ...
    print(
        "Finished estimating for education type = "
        + str(edType)
        + ". Optimal beta and spread are:"
    )
    ...
    with open(df_resFileStr, mode) as f:
        outStr = repr(
            {"EducationGroup": edType, "beta": opt_params[0], "nabla": opt_params[1]}
        )
        f.write(outStr + "\n")
        f.close()
...
if IncUnemp == 0.7 and IncUnempNoBenefits == 0.5:
    # Baseline unemployment system:
    print(
        "Calculating all results for CRRA = "
        + str(round(CRRA, 1))
        + " and R = "
        + str(round(Rfree_base[0], 3))
        + ":\n"
    ...
    ar_resFileStr = (
        res_dir + "/AllResults_CRRA_" + str(CRRA) + "_R_" + str(Rfree_base[0]) + ".txt"
    )
else:
    print(
        "Calculating all results for an alternativ unemployment system with IncUnemp = "
        + str(round(IncUnemp, 2))
        + " and IncUnempNoBenefits = "
        + str(round(IncUnempNoBenefits, 2))
        + ":\n"
...
while readStr != "":
    ...
    betasObjFuncEduc(
        ...
        print_mode=True,
        print_file=True,
        filename=ar_resFileStr,
    )
    checkDiscFacDistribution(
        beta, nabla, edType, print_mode=True, print_file=True, filename=ar_resFileStr
    )
    readStr = betFile.readline().strip()
betFile.close()
...
betasObjFunc(
    [myEstim[0][0], myEstim[1][0], myEstim[2][0]],
...
if run_additional_analysis:
    ...
    for e in [0, 1, 2]:
        dfs = Uniform(
            myEstim[e][0] - myEstim[e][1], myEstim[e][0] + myEstim[e][1]
        ).approx(DiscFacCount)
