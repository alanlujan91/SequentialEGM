...
import numpy as np
from EstimParameters import T_sim, makeCondMrkvArrays_base, makeFullMrkvArray
    ...
    def hitWithRecessionShock(self, shock_type):
        ...
        if shock_type == "recessionTaxCut":
            tax_cut_states = np.logical_and(
                np.greater(self.history["MrkvNow"], 2 * self.num_base_MrkvStates),
                np.less(self.history["MrkvNow"], 9 * 2 * self.num_base_MrkvStates),
            )  # $$$$$$$$$$ assumes all markov states above 11 and below 36 are tax cut states
            tax_cut_multiplier[tax_cut_states] *= self.TaxCutIncFactor
        elif shock_type == "recessionCheck":
            CheckAmount = (
                np.equal(self.history["MrkvNow"], 3 * self.num_base_MrkvStates)
                * self.CheckStimLvl
            )  # only if MrkvState is 3
...
def solveAggConsMarkovALT(
    ...
    for j in range(StateCount):
        ...
        mNrmNext_array = (
            Rfree[j] * aNrmNow_tiled / PermShkValsNext_tiled + TranShkValsNext_tiled
        )
