def returnParameters(Parametrization="Baseline", OutputFor="_Main.py"):
    ...
    if top_most_folder == "FromPandemicCode":
        Abs_Path_Results = "".join(
            [x + "//" for x in folders[0:-1]],
        )
        Abs_Path = cwd
    else:
    ...
    Splurge_txt_location = (
        Abs_Path_Results + "/Target_AggMPCX_LiquWealth/Result_CRRA_2.0.txt"
    )
    ...
    if Parametrization == "ADElas" or Parametrization == "ADElas_PVSame":
        ...
        CRRA = 1.0
        betas_txt_location = (
            Abs_Path_Results + "/Results/DiscFacEstim_CRRA_1.0_R_1.01.txt"
        )
        Splurge_txt_location = (
            Abs_Path_Results + "/Target_AggMPCX_LiquWealth/Result_CRRA_1.0.txt"
        )
    elif Parametrization == "CRRA3" or Parametrization == "CRRA3_PVSame":
        CRRA = 3.0
        betas_txt_location = (
            Abs_Path_Results + "/Results/DiscFacEstim_CRRA_3.0_R_1.01.txt"
        )
        Splurge_txt_location = (
            Abs_Path_Results + "/Target_AggMPCX_LiquWealth/Result_CRRA_3.0.txt"
        )
    elif Parametrization == "Rfree_1005" or Parametrization == "Rfree_1005_PVSame":
        Rfree_base = [1.005]
        betas_txt_location = (
            Abs_Path_Results + "/Results/DiscFacEstim_CRRA_2.0_R_1.005.txt"
        )
    elif Parametrization == "Rfree_1015" or Parametrization == "Rfree_1015_PVSame":
        Rfree_base = [1.015]
        betas_txt_location = (
            Abs_Path_Results + "/Results/DiscFacEstim_CRRA_2.0_R_1.015.txt"
        )
        ...
        betas_txt_location = (
            Abs_Path_Results + "/Results/DiscFacEstim_CRRA_2.0_R_1.01_altBenefits.txt"
        )
        IncUnempNoBenefits = 0.15
        IncUnemp = 0.3

    myEstim = [[], [], []]
    f = open(betas_txt_location)
    ...
    if Parametrization.find("PVSame") > 0:
        ...
        NPV_AddInc_Check = getSimulationDiff(
            base_results, check_results, "NPV_AggIncome"
        )
        NPV_AddInc_UI = getSimulationDiff(
            base_results, UI_results, "NPV_AggIncome"
        )  # Policy expenditure
        NPV_AddInc_TaxCut = getSimulationDiff(
            base_results, TaxCut_results, "NPV_AggIncome"
        )
    ...
    DiscFacMeanH = myEstim[1][
        0
    ]  # Mean intertemporal discount factor for high school types
    ...
    DiscFacCount = 7
    DiscFacDstnD = Uniform(
        DiscFacMeanD - DiscFacSpreadD, DiscFacMeanD + DiscFacSpreadD
    ).approx(DiscFacCount)
    DiscFacDstnH = Uniform(
        DiscFacMeanH - DiscFacSpreadH, DiscFacMeanH + DiscFacSpreadH
    ).approx(DiscFacCount)
    DiscFacDstnC = Uniform(
        DiscFacMeanC - DiscFacSpreadC, DiscFacMeanC + DiscFacSpreadC
    ).approx(DiscFacCount)
    ...
    GICmaxBetas = [
        (PermGroFac_base_d[0] ** CRRA) / Rfree_base[0],
        (PermGroFac_base_h[0] ** CRRA) / Rfree_base[0],
    ...
    Uspell_recession = (
        4  # Average duration of unemployment spell in recession, in quarters
    )
    ...
    PolicyUBspell = (
        2  # Average duration that policy of extended unemployment benefits is in place
    )
    # Tax Cut parameter
    ...
    CheckStimLvl_PLvl_Cutoff_start = (
        100 / 4 / 1
    )  # 100 k yearly income #At this Level of permanent inc, Stimulus beings to fall linearly
    CheckStimLvl_PLvl_Cutoff_end = (
        150 / 4 / 1
    )  # 150k yearly income #At this Level of permanent inc, Stimulus is zero
    ...
    aXtraNestFac = (
        3  # Exponential nesting factor for aXtraGrid (how dense is grid near zero)
    )
    ...
    num_base_MrkvStates = (
        2 + UBspell_normal
    )  # employed, unemployed with 2 quarters benefits, unemployed with 1 quarter benefit, unemployed no benefits
    ...
    def makeMacroMrkvArray_recession(Rspell, num_experiment_periods):
        ...
        MacroMrkvArray = np.zeros(
            (2 * (num_experiment_periods + 1), 2 * (num_experiment_periods + 1))
        )
        MacroMrkvArray[0:2, 0:2] = recession_transitions
        for i in np.array(range(num_experiment_periods - 1)) + 1:
            MacroMrkvArray[
                2 * i : 2 * i + 2, 2 * i + 2 : 2 * i + 4
            ] = recession_transitions
        MacroMrkvArray[
            2 * num_experiment_periods : 2 * num_experiment_periods + 2, 0:2
        ] = recession_transitions
    ...
    def makeCondMrkvArrays_base(Urate_normal, Uspell_normal, UBspell_normal):
        U_persist_normal = 1.0 - 1.0 / Uspell_normal
        E_persist_normal = 1.0 - Urate_normal * (1.0 - U_persist_normal) / (
            1.0 - Urate_normal
        )
        MrkvArray_normal = small_MrkvArray(
            E_persist_normal, U_persist_normal, UBspell_normal
        )
        CondMrkvArrays = [MrkvArray_normal]
        return CondMrkvArrays

    def makeCondMrkvArrays_recession(
        Urate_normal,
        Uspell_normal,
        UBspell_normal,
        Urate_recession,
        ...
        U_persist_recession = 1.0 - 1.0 / Uspell_recession
        E_persist_recession = 1.0 - Urate_recession * (1.0 - U_persist_recession) / (
            1.0 - Urate_recession
        )
        MrkvArray_normal = small_MrkvArray(
            E_persist_normal, U_persist_normal, UBspell_normal
        )
        MrkvArray_recession = small_MrkvArray(
            E_persist_recession, U_persist_recession, UBspell_normal
        )
        CondMrkvArrays = [MrkvArray_normal, MrkvArray_recession] * (
            num_experiment_periods + 1
        )
        return CondMrkvArrays

    def makeCondMrkvArrays_recessionUI(
        Urate_normal,
        Uspell_normal,
        UBspell_normal,
        Urate_recession,
        ...
        MrkvArray_normalUI = small_MrkvArray(
            E_persist_normal, U_persist_normal, UBspell_normal, transition_ub=False
        )
        MrkvArray_recessionUI = small_MrkvArray(
            E_persist_recession,
            U_persist_recession,
            UBspell_normal,
            transition_ub=False,
        ...
        CondMrkvArrays = (
            [MrkvArray_normal, MrkvArray_recession]
            + [MrkvArray_normalUI, MrkvArray_recessionUI] * ExtraUBperiods
            + [MrkvArray_normal, MrkvArray_recession]
            * (num_experiment_periods - ExtraUBperiods)
        )
        return CondMrkvArrays
    ...
    def makeFullMrkvArray(MacroMrkvArray, CondMrkvArrays):
        for i in range(MacroMrkvArray.shape[0]):
            ...
            for j in range(MacroMrkvArray.shape[0] - 1):
                this_row = np.concatenate(
                    (this_row, MacroMrkvArray[i, j + 1] * CondMrkvArrays[j + 1]), axis=1
                )
            if i == 0:
                FullMrkv = this_row
            else:
    ...
    CondMrkvArrays_base_d = makeCondMrkvArrays_base(
        Urate_normal_d, Uspell_normal, UBspell_normal
    )
    CondMrkvArrays_base_h = makeCondMrkvArrays_base(
        Urate_normal_h, Uspell_normal, UBspell_normal
    )
    CondMrkvArrays_base_c = makeCondMrkvArrays_base(
        Urate_normal_c, Uspell_normal, UBspell_normal
    )
    ...
    MacroMrkvArray_recession = makeMacroMrkvArray_recession(
        Rspell, num_experiment_periods
    )
    CondMrkvArrays_recession_d = makeCondMrkvArrays_recession(
        Urate_normal_d,
        Uspell_normal,
        UBspell_normal,
        Urate_recession_d,
    ...
    CondMrkvArrays_recession_h = makeCondMrkvArrays_recession(
        Urate_normal_h,
        Uspell_normal,
        UBspell_normal,
        Urate_recession_h,
    ...
    CondMrkvArrays_recession_c = makeCondMrkvArrays_recession(
        Urate_normal_c,
        Uspell_normal,
        UBspell_normal,
        Urate_recession_c,
    ...
    MrkvArray_recession_d = makeFullMrkvArray(
        MacroMrkvArray_recession, CondMrkvArrays_recession_d
    )
    MrkvArray_recession_h = makeFullMrkvArray(
        MacroMrkvArray_recession, CondMrkvArrays_recession_h
    )
    MrkvArray_recession_c = makeFullMrkvArray(
        MacroMrkvArray_recession, CondMrkvArrays_recession_c
    )
    ...
    MrkvArray_recessionCheck_d = makeFullMrkvArray(
        MacroMrkvArray_recessionCheck, CondMrkvArrays_recessionCheck_d
    )
    MrkvArray_recessionCheck_h = makeFullMrkvArray(
        MacroMrkvArray_recessionCheck, CondMrkvArrays_recessionCheck_h
    )
    MrkvArray_recessionCheck_c = makeFullMrkvArray(
        MacroMrkvArray_recessionCheck, CondMrkvArrays_recessionCheck_c
    )
    ...
    MrkvArray_recessionTaxCut_d = makeFullMrkvArray(
        MacroMrkvArray_recessionTaxCut, CondMrkvArrays_recessionTaxCut_d
    )
    MrkvArray_recessionTaxCut_h = makeFullMrkvArray(
        MacroMrkvArray_recessionTaxCut, CondMrkvArrays_recessionTaxCut_h
    )
    MrkvArray_recessionTaxCut_c = makeFullMrkvArray(
        MacroMrkvArray_recessionTaxCut, CondMrkvArrays_recessionTaxCut_c
    )

    MacroMrkvArray_recessionUI = MacroMrkvArray_recession
    CondMrkvArrays_recessionUI_d = makeCondMrkvArrays_recessionUI(
        Urate_normal_d,
        Uspell_normal,
        UBspell_normal,
        Urate_recession_d,
        Uspell_recession,
        num_experiment_periods,
        UBspell_extended - UBspell_normal,
    )
    CondMrkvArrays_recessionUI_h = makeCondMrkvArrays_recessionUI(
        Urate_normal_h,
        Uspell_normal,
        UBspell_normal,
        Urate_recession_h,
    ...
    CondMrkvArrays_recessionUI_c = makeCondMrkvArrays_recessionUI(
        Urate_normal_c,
        Uspell_normal,
        UBspell_normal,
        Urate_recession_c,
    ...
    MrkvArray_recessionUI_d = makeFullMrkvArray(
        MacroMrkvArray_recessionUI, CondMrkvArrays_recessionUI_d
    )
    MrkvArray_recessionUI_h = makeFullMrkvArray(
        MacroMrkvArray_recessionUI, CondMrkvArrays_recessionUI_h
    )
    MrkvArray_recessionUI_c = makeFullMrkvArray(
        MacroMrkvArray_recessionUI, CondMrkvArrays_recessionUI_c
    )

    # find intial distribution of states for each education type
    vals_d, vecs_d = np.linalg.eig(np.transpose(MrkvArray_base_d[0]))
    dist_d = np.abs(np.abs(vals_d) - 1.0)
    ...
    dist_h = np.abs(np.abs(vals_h) - 1.0)
    ...
    dist_c = np.abs(np.abs(vals_c) - 1.0)
    ...
    init_dropout = {
        "cycles": 0,  # 00This will be overwritten at type construction
        "T_cycle": T_cycle,
    ...
    base_dict = {"shock_type": "base", "UpdatePrb": 1.0, "Splurge": Splurge}
    ...
    intercept_prev = np.ones(
        (num_base_MrkvStates, num_base_MrkvStates)
    )  # Intercept of aggregate savings function
    slope_prev = np.zeros(
        (num_base_MrkvStates, num_base_MrkvStates)
    )  # Slope of aggregate savings function
