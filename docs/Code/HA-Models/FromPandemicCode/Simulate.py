def Simulate(Run_Dict, figs_dir, Parametrization="Baseline"):
    ...
    from OtherFunctions import saveAsPickle, saveAsPickleUnderVarName
    ...
    ] = returnParameters(Parametrization=Parametrization, OutputFor="_Main.py")
    ...
    for ThisType in BaseTypeList:
        ...
        IncomeDstn_recession = [
            ThisType.IncomeDstn[0] * (2 * (num_experiment_periods + 1))
        ]  # for normal, rec, recovery
    ...
    Rspell = AggDemandEconomy.agents[
        0
    ].Rspell  # NOTE - this should come from the market, not the agent
    ...
    if Run_Baseline:
        ...
        t0 = time()
        base_results = AggDemandEconomy.runExperiment(
            **base_dict_agg, Full_Output="ForWelfare"
        )
    ...
    def runExperimentsAllRecessions(dict_changes, AggDemandEconomy):
        ...
        for t in range(max_recession_duration):
            dictt["EconomyMrkv_init"] = (
                list(np.arange(1, AggDemandEconomy.num_experiment_periods + 1) * 2)
                + [0] * 20
            )
            dictt["EconomyMrkv_init"][0 : t + 1] = (
                np.array(dictt["EconomyMrkv_init"][0 : t + 1]) + 1
            )
            print(dictt["EconomyMrkv_init"])
            this_result = AggDemandEconomy.runExperiment(
                **dictt, Full_Output="ForWelfare"
            )
    ...
    def Run_FullRoutine(shock_type):
        ...
        if Run_NonAD:
            ...
            AggDemandEconomy_Routine.switch_shock_type(shock_type)
            AggDemandEconomy_Routine.solve()
            [results, all_results] = runExperimentsAllRecessions(
                changes, AggDemandEconomy_Routine
            )
            saveAsPickle(shock_type + "_results", results, figs_dir)
        ...
        if Run_AD:
            ...
            AggDemandEconomy_Routine.switch_shock_type(shock_type)
            if shock_type == "recession":
                AggDemandEconomy_Routine.solveAD_Recession(
                    num_max_iterations=num_max_iterations_solvingAD,
                    convergence_cutoff=convergence_tol_solvingAD,
                    name=shock_type,
                )
            elif shock_type == "recessionCheck":
                AggDemandEconomy_Routine.solveAD_Check_Recession(
                    num_max_iterations=num_max_iterations_solvingAD,
                    convergence_cutoff=convergence_tol_solvingAD,
                    name=shock_type,
                )
            elif shock_type == "recessionUI":
                AggDemandEconomy_Routine.solveAD_UIExtension_Recession(
                    num_max_iterations=num_max_iterations_solvingAD,
                    convergence_cutoff=convergence_tol_solvingAD,
                    name=shock_type,
                )
            elif shock_type == "recessionTaxCut":
                AggDemandEconomy_Routine.solveAD_Recession_TaxCut(
                    num_max_iterations=num_max_iterations_solvingAD,
                    convergence_cutoff=convergence_tol_solvingAD,
                    name=shock_type,
                )
            t1 = time()
            print(
                "Solving took " + mystr(t1 - t0) + " seconds for shock_type: ",
                shock_type,
            )
            ...
            [results_AD, all_results_AD] = runExperimentsAllRecessions(
                changes, AggDemandEconomy_Routine
            )
        ...
        if Run_1stRoundAD:
            ...
            print(
                "Solving took "
                + mystr(t1 - t0)
                + " seconds for 1st round AD for shock_type: ",
                shock_type,
            )
            ...
            [
                results_firstRoundAD,
                all_results_firstRoundAD,
            ] = runExperimentsAllRecessions(changes, AggDemandEconomy_Routine)
            saveAsPickle(
                shock_type + "_results_firstRoundAD", results_firstRoundAD, figs_dir
            )
            saveAsPickle(
                shock_type + "_all_results_firstRoundAD",
                all_results_firstRoundAD,
                figs_dir,
            )
