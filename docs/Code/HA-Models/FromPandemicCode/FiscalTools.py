...
def runExperiment(
    ...
    TranShk_all = np.concatenate(
        [ThisType.history["TranShkNow"] for ThisType in agents], axis=1
    )
    mNrm_all = np.concatenate(
        [ThisType.history["mNrmNow"] for ThisType in agents], axis=1
    )
    aNrm_all = np.concatenate(
        [ThisType.history["aNrmNow"] for ThisType in agents], axis=1
    )
