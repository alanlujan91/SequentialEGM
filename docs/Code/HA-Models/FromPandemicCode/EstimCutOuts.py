...
def betasObjFunc(betas, spread, print_mode=False):
    ...
    sumSquares_d = (
        np.sum((np.array(Stats_d.LorenzPts) - data_LorenzPts[0]) ** 2)
        + (Stats_d.avgLWPI - data_avgLWPI[0]) ** 2
    )
    sumSquares_h = (
        np.sum((np.array(Stats_h.LorenzPts) - data_LorenzPts[1]) ** 2)
        + (Stats_h.avgLWPI - data_avgLWPI[1]) ** 2
    )
    sumSquares_c = (
        np.sum((np.array(Stats_c.LorenzPts) - data_LorenzPts[2]) ** 2)
    ...
    if print_mode:
        ...
        print(
            ...
            + mystr(Stats_d.avgLWPI)
            + " H = "
            + mystr(Stats_h.avgLWPI)
            + " C = "
            + mystr(Stats_c.avgLWPI)
        )
        print("Distance = " + mystr(distance))
        print(
            "Total LW/Total PI (not targeted): D = "
            + mystr(Stats_d.LWoPI)
            + " H = "
            + mystr(Stats_h.LWoPI)
            + " C = "
