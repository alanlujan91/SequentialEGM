...
data_avgLWPI = (
    np.array([15.7, 47.7, 111]) * 4
)  # weighted average of fractions in percent
...
data_medianLWPI = (
    np.array([1.16, 7.55, 28.2]) * 4
)  # weighted median of fractions in percent
...
data_EducShares = [
    ...
    0.527,
    0.38,
]  # Proportion of dropouts, HS grads, college types, SCF 2004
# Wealth share of each type
data_WealthShares = (
    np.array([0.008, 0.179, 0.812]) * 100
)  # Percentage of total wealth of dropouts, HS grads, college types, SCF 2004
...
Urate_normal_h = (
    0.044  # Unemployment rate in normal times, highschooler+some college 2004
)
Urate_normal_c = 0.027  # Unemployment rate in normal times, college 2004

Uspell_normal = (
    1.5  # Average duration of unemployment spell in normal times, in quarters
)
UBspell_normal = (
    2  # Average duration of unemployment benefits in normal times, in quarters
)
...
f = open("../Target_AggMPCX_LiquWealth/Result_CRRA_" + str(CRRA) + ".txt")
...
IncUnemp = (
    0.7  # Unemployment benefits replacement rate (proportion of permanent income)
)
IncUnempNoBenefits = (
    0.5  # Unemployment income when benefits run out (proportion of permanent income)
)
if len(sys.argv) >= 5:
...
pLvlInitMean_d = np.log(
    6.2
)  # Average quarterly permanent income of "newborn" HS dropout ($1000)
pLvlInitMean_h = np.log(
    11.1
)  # Average quarterly permanent income of "newborn" HS graduate ($1000)
pLvlInitMean_c = np.log(
    14.5
)  # Average quarterly permanent income of "newborn" HS  ($1000)
...
init_mrkv_dist_d = vecs_d[:, idx_d].astype(float) / np.sum(
    vecs_d[:, idx_d].astype(float)
)
...
init_mrkv_dist_h = vecs_h[:, idx_h].astype(float) / np.sum(
    vecs_h[:, idx_h].astype(float)
)

vals_c, vecs_c = np.linalg.eig(np.transpose(MrkvArray_base_c[0]))
dist_c = np.abs(np.abs(vals_c) - 1.0)
idx_c = np.argmin(dist_c)
init_mrkv_dist_c = vecs_c[:, idx_c].astype(float) / np.sum(
    vecs_c[:, idx_c].astype(float)
)

# Define a parameter dictionary for dropout type
init_dropout = {
    "cycles": 0,  # This will be overwritten at type construction
    "T_cycle": T_cycle,
...
frictionless_changes = {"UpdatePrb": 1.0}
