...
class StickyEconsumerType(AggShockConsumerType):
    ...
    def getShocks(self):
        ...
        super(
            self.__class__, self
        ).getShocks()  # Get permanent and transitory combined shocks
        newborns = self.t_age == 0
        self.shocks["TranShkNow"][newborns] = (
            self.TranShkAggNow * self.wRteNow
        )  # Turn off idiosyncratic shocks for newborns
        ...
        PermShkPcvd[self.update] *= self.pLvlErrNow[
            self.update
        ]  # Updaters see the true permanent shock and all missed news
        ...
        if not hasattr(
            self, "parker_experiment"
        ):  # This attribute is added for an experiment
            return
        if not hasattr(
            self, "noticed"
        ):  # If the "noticed" attribute does not exist, initialize it
            self.noticed = np.zeros(self.AgentCount, dtype=bool)
        if self.BonusLvl > 0.0:
            if (
                self.t_until_bonus > 0
            ):  # Which agents "notice" the future bonus this period
            ...
            BonusNrm[noticers] = self.BonusLvl / (
                self.pLvlNow[noticers] * self.shocks["PermShkNow"][noticers]
            )
            self.shocks["TranShkNow"] += BonusNrm * self.getRfree() ** (
                -self.t_until_bonus
            )
    ...
    def getStates(self):
        ...
        mNrmPcvdNow = (
            mLvlTrueNow / self.pLvlNow
        )  # This is perceived normalized resources
    ...
    def getMaggNow(self):
        ...
        """
        MaggPcvdNow = (
            self.MaggNow * self.pLvlErrNow
        )  # Agents know the true level of aggregate market resources,
...
class StickyEmarkovConsumerType(AggShockMarkovConsumerType, StickyEconsumerType):
    ...
    def getpLvlError(self):
        ...
        pLvlErr = np.ones(self.AgentCount)
        pLvlErr[self.dont] = (
            self.PermShkAggNow / self.PermGroFacAgg[self.MrkvNowPcvd[self.dont]]
        )
        return pLvlErr
...
class StickySmallOpenMarkovEconomy(SmallOpenMarkovEconomy):
    ...
    def runParkerExperiment(self, T_sim):
        ...
        for t in range(T_sim):
            ...
            cLvl_new = np.concatenate(
                [self.agents[i].cLvlNow for i in range(len(self.agents))]
            )
