...
from StickyEparams import PermShkAggVar, UpdatePrb, figures_dir, results_dir, tables_dir
...
def runParkerExperiment(BaseEconomy, BonusSize, T_before_bonus, T_after_bonus, verbose):
    ...
    BonusLvl = (
        BonusSize * BaseEconomy.wRte * pLvlBase
    )  # Bonus check of 5% of average quarterly income
...
def makeParkerExperimentText(
    BonusLvl, T_ahead, cLvlNone_hist, cLvlBonus_hist, sticky_bool, out_filename=None
):
    """
    ...
    output = (
        "The bonus check is announced to "
        + agent_text
        + number_text[T_ahead]
        + period_text
        + " in advance.\n"
    ...
    for t in range(-T_ahead, T_total - T_ahead):
        ...
        text_now += con_text1
        text_now += f"{(cLvlBonus_hist[s] - cLvlNone_hist[s]) / BonusLvl:.1%}"
        text_now += con_text2
        text_now += "\n"
        output += text_now
    ...
    for i in range(4):
        ...
        for n in range(num_agg_sims):
            ...
            for k in range(len(Economy.agents)):
                Economy.agents[k].getEconomyData(
                    Economy
