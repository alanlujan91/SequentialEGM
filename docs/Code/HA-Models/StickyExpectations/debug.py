...
for i in [3]:
    ...
    for t in range(T_sim):
        ...
        cLvl_new = np.concatenate(
            [Economy.agents[j].cLvlNow for j in range(len(Economy.agents))]
        )
        cLvlMean_hist[t] = np.mean(cLvl_new)
        pLvl_new = np.concatenate(
            [np.array([Economy.agents[j].pLvlNow]) for j in range(len(Economy.agents))]
        )
        pLvlMean_hist[t] = np.mean(pLvl_new)
        tran_new = np.concatenate(
            [Economy.agents[j].TranShkNow for j in range(len(Economy.agents))]
        )
        tranMean_hist[t] = np.mean(tran_new)

        tran_new = np.concatenate(
            [np.array([Economy.agents[j].MrkvNow]) for j in range(len(Economy.agents))]
        )
        tranMean_hist[t] = np.mean(tran_new)
