...
from StickyEmodel import (
    StickyCobbDouglasMarkovEconomy,
    StickyEmarkovConsumerType,
    StickyEmarkovRepAgent,
    StickySmallOpenMarkovEconomy,
)
from StickyEtools import (
    extractSampleMicroData,
    ...
    makeStickyEdataFile,
    makeuCostVsPiFig,
    makeValueVsAggShkVarFig,
    makeValueVsPiFig,
    runParkerExperiment,
    runStickyEregressionLagged,
    runStickyEregressions,
    runStickyEregressionsInStata,
)

# Check RAM size and warn user if less than 32G:
memory_specs = psutil.virtual_memory()
actual_RAM_size_GB = int(round(memory_specs.total / (2.0**30)))
desired_RAM_size_GB = 32
warning_message_1 = (
    """\n\nWARNING: You have selected to run SOE and/or DSGE, with RAM < """
    + str(desired_RAM_size_GB)
    + """GB. This may fill up your memory and crash your computer. Please run this code on a machine with >= """
    + str(desired_RAM_size_GB)
    + """G of RAM. Relevant values:
...
warning_message_2 = (
    """\n\nWARNING: You have selected to run both SOE and DSGE, with RAM < """
    + str(2 * desired_RAM_size_GB)
    + """GB but > """
    + str(desired_RAM_size_GB)
    + "GB.\n"
)
...
ignore_periods = (
    Params.ignore_periods
)  # Number of simulated periods to ignore as a "burn-in" phase
interval_size = (
    Params.interval_size
)  # Number of periods in each non-overlapping subsample
total_periods = Params.periods_to_sim  # Total number of periods in simulation
interval_count = (
    total_periods - ignore_periods
) // interval_size  # Number of intervals in the macro regressions
periods_to_sim_micro = (
    Params.periods_to_sim_micro
)  # To save memory, micro regressions are run on a smaller sample
AgentCount_micro = (
    Params.AgentCount_micro
)  # To save memory, micro regressions are run on a smaller sample
my_counts = [interval_size, interval_count]
long_counts = [interval_size * interval_count, 1]
mystr = lambda number: f"{number:.3f}"
...
if use_stata:
    runRegressions = lambda a, b, c, d, e: runStickyEregressionsInStata(
        a, b, c, d, e, stata_exe
    )
else:
...
if __name__ == "__main__":
    ...
    if do_SOE:
        if run_models:
            ...
            StickySOmarkovEconomy = StickySmallOpenMarkovEconomy(
            ...
            t_end = time()
            print(
                "Solving the small open Markov economy took "
                + mystr(t_end - t_start)
                + " seconds."
            )
            ...
            t_end = time()
            print(
                "Simulating the sticky small open Markov economy took "
                + mystr(t_end - t_start)
                + " seconds."
            )
            ...
            desc = (
                "Results for the sticky small open Markov economy with update probability "
                + mystr(Params.UpdatePrb)
            )
            ...
            if calc_micro_stats:
                sticky_SOEmarkov_micro_data = extractSampleMicroData(
                    ...
                    np.minimum(
                        StickySOmarkovEconomy.agents[0].AgentCount, AgentCount_micro
                    ),
                    ignore_periods,
                )
            DeltaLogC_stdev = np.genfromtxt(
                results_dir + "SOEmarkovStickyResults.csv", delimiter=","
            )[
                3
            ]  # For use in frictionless spec
            ...
            if calc_micro_stats:
                ...
                makeMicroRegressionTable(
                    "CGrowCross",
                    [frictionless_SOEmarkov_micro_data, sticky_SOEmarkov_micro_data],
                )
            ...
            if run_alt_beliefs:
                ...
                cFunc_original_list = []
                for n in range(TypeCount):
                    cFunc_original_list.append(
                        deepcopy(StickySOEmarkovConsumers[n].solution[0].cFunc)
                    )
                ...
                t_end = time()
                print(
                    "Re-solving the small open Markov economy with alternate beliefs took "
                    + mystr(t_end - t_start)
                    + " seconds."
                )
                ...
                t_end = time()
                print(
                    "Simulating the alternate belief small open Markov economy took "
                    + mystr(t_end - t_start)
                    + " seconds."
                )

                desc = (
                    "Results for the alternate belief small open Markov economy with update probability "
                    + mystr(Params.UpdatePrb)
                )
                ...
                AgentCount = np.sum(
                    [StickySOEmarkovConsumers[n].AgentCount for n in range(TypeCount)]
                )
                ...
                for j in range(Params.StateCount):
                    ...
                    for n in range(TypeCount):
                        ...
                        cAlt[start:end, j] = (
                            StickySOEmarkovConsumers[n]
                            .solution[0]
                            .cFunc[j](mNrm_temp, Mnrm_temp)
                        )
                        cBase[start:end, j] = cFunc_original_list[n][j](
                            mNrm_temp, Mnrm_temp
                        )
                        start = end
                ...
                AltBeliefResultString += (
                    "The mean difference between baseline and alternate belief consumption functions is "
                    + f"{np.mean(cAltVsBasePctDiff):.4%}"
                    + ".\n"
                )
                AltBeliefResultString += (
                    "The stdev of differences between baseline and alternate belief consumption functions is "
                    + f"{np.std(cAltVsBasePctDiff):.4%}"
                    + ".\n"
                )
                AltBeliefResultString += (
                    "The highest percentage difference between baseline and alternate belief consumption functions is "
                    + f"{np.max(cAltVsBasePctDiff):.4%}"
                    + ".\n"
                )
                AltBeliefResultString += (
                    "The lowest percentage difference between baseline and alternate belief consumption functions is "
                    + f"{np.min(cAltVsBasePctDiff):.4%}"
                    + ".\n"
                )
                AltBeliefResultString += (
                    "The fraction of absolute differences less than "
                    + f"{my_tol:.2%}"
                    + " is "
                    + f"{np.mean(np.abs(cAltVsBasePctDiff) < my_tol):.2%}"
                    + ".\n"
                )
            ...
            if run_ucost_vs_pi:
                ...
                vBirth_F = np.genfromtxt(
                    results_dir + "SOEmarkovFrictionlessBirthValue.csv", delimiter=","
                )
                ...
                for j in range(UpdatePrbVec.size):
                    ...
                    vBirth_S = np.genfromtxt(
                        results_dir + "TEMPBirthValue.csv", delimiter=","
                    )
                    uCost = np.mean(1.0 - (vBirth_S / vBirth_F) ** (1.0 / (1.0 - CRRA)))
                    uCostVec[j] = uCost
        ...
        if make_tables:
            t_start = time()
            frictionless_panel = runRegressions(
                "SOEmarkovFrictionlessData", interval_size, False, False, True
            )
            frictionless_me_panel = runRegressions(
                "SOEmarkovFrictionlessData", interval_size, True, False, True
            )
            frictionless_long_panel = runRegressions(
                "SOEmarkovFrictionlessData",
                interval_size * interval_count,
                True,
                False,
                True,
            )
            sticky_panel = runRegressions(
                "SOEmarkovStickyData", interval_size, False, True, True
            )
            sticky_me_panel = runRegressions(
                "SOEmarkovStickyData", interval_size, True, True, True
            )
            sticky_alt_panel = runRegressions(
                "SOEmarkovStickyAltData", interval_size, True, True, True
            )
            sticky_long_panel = runRegressions(
                "SOEmarkovStickyData", interval_size * interval_count, True, True, True
            )
            if not run_parker:
                makeResultsTable(
                    "Aggregate Consumption Dynamics in SOE Model",
                ...
                makeResultsTable(
                    "Aggregate Consumption Dynamics in SOE Model (Alternate Beliefs)",
                    [frictionless_me_panel, sticky_alt_panel],
                    my_counts,
                    "SOEmrkvSimRegAlt",
                    "tPESOEsimAlt",
                )
                makeResultsTable(
                    "Aggregate Consumption Dynamics in SOE Model",
                    [frictionless_panel, sticky_panel],
                    my_counts,
                    "SOEmrkvSimRegNoMeasErr",
                    "tPESOEsimNoMeasErr",
                )
                makeResultsTable(
                    "Aggregate Consumption Dynamics in SOE Model",
                ...
                makeResultsTable(
                    None,
                    [frictionless_me_panel],
                    my_counts,
                    "SOEmrkvSimRegF",
                    "tPESOEsimF",
                )
                makeResultsTable(
                    None, [sticky_me_panel], my_counts, "SOEmrkvSimRegS", "tPESOEsimS"
                )
            else:
                makeResultsTable(
                    "Aggregate Consumption Dynamics in SOE Model (Parker Experiment)",
                    [frictionless_me_panel, sticky_me_panel],
                    my_counts,
                    "SOEmrkvSimRegHetero",
                    "tPESOEsimHetero",
            ...
            lag_coeffs_array = runStickyEregressionLagged(
                "SOEmarkovStickyData", interval_size, False, True, True
            )
            lag_coeffs_array_me = runStickyEregressionLagged(
                "SOEmarkovStickyData", interval_size, True, True, True
            )
        ...
        if run_parker and run_models:
            ...
            Shk_idx_reset = np.where(StickySOmarkovEconomy.MrkvNow_hist == MrkvNow)[0][
                0
            ]
            ...
            runParkerExperiment(
                StickySOmarkovEconomy, 0.05, 1, 4, True
            )  # One quarter ahead
            runParkerExperiment(
                StickySOmarkovEconomy, 0.05, 2, 4, True
            )  # Two quarters ahead
            runParkerExperiment(
                StickySOmarkovEconomy, 0.05, 3, 4, True
            )  # Three quarters ahead

            t_end = time()
            print(
                'Running the "Parker experiment" took '
                + str(t_end - t_start)
                + " seconds."
            )
    ...
    if do_DSGE:
        if run_models:
            if do_SOE and (actual_RAM_size_GB < 2 * desired_RAM_size_GB):
                warnings.warn(
                    warning_message_2
                )  # Tell the user that micro data from SOE model is being deleted for memory reasons
                attr_list = [
                    "aLvlNow",
                    "cLvlNow",
                    "yLvlNow",
            ...
            StickyDSGEmarkovEconomy = StickyCobbDouglasMarkovEconomy(
                agents=StickyDSGEmarkovConsumers, **Params.init_DSGE_mrkv_market
            )
            ...
            t_end = time()
            print(
                "Solving the sticky Cobb-Douglas Markov economy took "
                + mystr(t_end - t_start)
                + " seconds."
            )
            ...
            desc = (
                "Results for the sticky Cobb-Douglas Markov economy with update probability "
                + mystr(Params.UpdatePrb)
            )
            ...
            DeltaLogC_stdev = np.genfromtxt(
                results_dir + "DSGEmarkovStickyResults.csv", delimiter=","
            )[
                3
            ]  # For use in frictionless spec
            if calc_micro_stats:
                sticky_DSGEmarkov_micro_data = extractSampleMicroData(
                    ...
                    np.minimum(
                        StickyDSGEmarkovEconomy.agents[0].AgentCount, AgentCount_micro
                    ),
                    ignore_periods,
                )
            ...
            if calc_micro_stats:
                StickyDSGEmarkovEconomy.overwrite_hist = (
                    True  # History will be overwritten by sticky outcomes
                )
            ...
            if calc_micro_stats:
                ...
                makeMicroRegressionTable(
                    "CGrowCrossDSGE",
                    [frictionless_DSGEmarkov_micro_data, sticky_DSGEmarkov_micro_data],
                )
        ...
        if make_tables:
            t_start = time()
            frictionless_panel = runRegressions(
                "DSGEmarkovFrictionlessData", interval_size, False, False, True
            )
            frictionless_me_panel = runRegressions(
                "DSGEmarkovFrictionlessData", interval_size, True, False, True
            )
            frictionless_long_panel = runRegressions(
                "DSGEmarkovFrictionlessData",
                interval_size * interval_count,
                True,
                False,
                True,
            ...
            sticky_panel = runRegressions(
                "DSGEmarkovStickyData", interval_size, False, True, True
            )
            sticky_me_panel = runRegressions(
                "DSGEmarkovStickyData", interval_size, True, True, True
            )
            sticky_long_panel = runRegressions(
                "DSGEmarkovStickyData", interval_size * interval_count, True, True, True
            )
            makeResultsTable(
                "Aggregate Consumption Dynamics in HA-DSGE Model",
                [frictionless_me_panel, sticky_me_panel],
                my_counts,
                "DSGEmrkvSimReg",
                "tDSGEsim",
            ...
            makeResultsTable(
                None, [frictionless_me_panel], my_counts, "DSGEmrkvSimRegF", "tDSGEsimF"
            )
            makeResultsTable(
                None, [sticky_me_panel], my_counts, "DSGEmrkvSimRegS", "tDSGEsimS"
            )
            t_end = time()
            print(
    ...
    if do_RA:
        if run_models:
            ...
            t_end = time()
            print(
                "Solving the representative agent Markov economy took "
                + mystr(t_end - t_start)
                + " seconds."
            )
            ...
            DeltaLogC_stdev = np.genfromtxt(
                results_dir + "RAmarkovStickyResults.csv", delimiter=","
            )[
                3
            ]  # For use in frictionless spec
        ...
        if make_tables:
            ...
            t_start = time()
            frictionless_panel = runRegressions(
                "RAmarkovFrictionlessData", interval_size, False, False, True
            )
            frictionless_me_panel = runRegressions(
                "RAmarkovFrictionlessData", interval_size, True, False, True
            )
            frictionless_long_panel = runRegressions(
                "RAmarkovFrictionlessData",
                interval_size * interval_count,
                True,
                False,
                True,
            ...
            sticky_panel = runRegressions(
                "RAmarkovStickyData", interval_size, False, True, True
            )
            sticky_me_panel = runRegressions(
                "RAmarkovStickyData", interval_size, True, True, True
            )
            sticky_long_panel = runRegressions(
                "RAmarkovStickyData", interval_size * interval_count, True, True, True
            )
            makeResultsTable(
                "Aggregate Consumption Dynamics in RA Model",
                [frictionless_me_panel, sticky_me_panel],
                my_counts,
                "RepAgentMrkvSimReg",
                "tRAsim",
    ...
    if make_tables:
        makeEquilibriumTable(
            "Eqbm",
            [
                "SOEmarkovFrictionless",
                "SOEmarkovSticky",
                "DSGEmarkovFrictionless",
                "DSGEmarkovSticky",
            ],
    ...
    if make_emp_table and use_stata:
        ...
        if stata_status != 0:
            raise ValueError(
                "Stata code could not run. Check stata_exe in USER_OPTIONS.py"
            )
    ...
    if make_histogram and use_stata:
        ...
        cmd = [
            stata_exe,
            "do",
            empirical_dir + "metaAnalysis/habitsHistogram.do",
            base_path,
        ]
        # Run Stata do-file
        stata_status = subprocess.call(cmd, shell="true")
