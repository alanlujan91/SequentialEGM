    ...
    if run_models:
        ...
        if run_parker or run_tax_cut:
            ...
            if run_parker:
                ...
                (
                    cLvl_StickyNone_parker,
                    cLvl_StickyBonus,
                    cLvl_FrictionlessNone_parker,
                    cLvl_FrictionlessBonus,
                ) = runParkerExperiment(
                    StickySOmarkovEconomy, 0.05, 1, 30, True
                )  # One quarter ahead
                t_end = time()
                print(
                ...
                plt.plot(
                    cLvl_FrictionlessBonus / cLvl_FrictionlessNone_parker,
                    label="Frictionless",
                )
            ...
            if run_tax_cut:
                ...
                (
                    cLvl_StickyTaxCut,
                    cLvl_StickyNone,
                    cLvl_FrictionlessTaxCut,
                    cLvl_FrictionlessNone,
                ) = runTaxCutExperiment(
                    StickySOmarkovEconomy, T_after=T_after, num_agg_sims=num_agg_sims
                )
                t_end = time()
                print(
                    'Running the "Tax Cut experiment" took '
                    + str(t_end - t_start)
                    + " seconds."
                )
                ...
                plt.plot(
                    cLvl_FrictionlessTaxCut / cLvl_FrictionlessNone,
                    label="Frictionless",
                )
                plt.legend(loc="upper right")
                plt.show()
            ...
            if run_parker and run_tax_cut:
                plt.plot(
                    cLvl_StickyBonus / cLvl_StickyNone_parker, label="Stimulus Payments"
                )
                plt.plot(
                    cLvl_StickyTaxCut[1:] / cLvl_StickyNone[1:], label="Income Tax Cut"
                )
                plt.legend(loc="upper right")
