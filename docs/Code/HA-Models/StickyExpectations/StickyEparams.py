...
)  # Absolute directory for primitive parameter files
tables_dir = os.path.join(
    my_file_path, "./Tables/"
)  # Absolute directory for saving tex tables
results_dir = os.path.join(
    my_file_path, "./Results/"
)  # Absolute directory for saving output files
figures_dir = os.path.join(
    my_file_path, "./Figures/"
)  # Absolute directory for saving figures
...
UpdatePrb = importParam(
    "UpdatePrb"
)  # Probability that each agent observes the aggregate productivity state each period (in sticky version)
UnempPrb = importParam("UnempPrb")  # Unemployment probability
DiePrb = importParam("DiePrb")  # Quarterly mortality probability
TranShkVarAnn = importParam(
    "TranShkVarAnn"
)  # Annual variance of idiosyncratic transitory shocks
PermShkVarAnn = importParam(
    "PermShkVarAnn"
)  # Annual variance of idiosyncratic permanent shocks
...
DeprFac = 1.0 - DeprFacAnn**0.25  # Quarterly depreciation rate
KSS = KtYratioSS = KYratioSS ** (
    1.0 / (1.0 - CapShare)
)  # Steady state Capital to labor productivity
wRteSS = (1.0 - CapShare) * KSS**CapShare  # Steady state wage rate
rFreeSS = CapShare * KSS ** (CapShare - 1.0)  # Steady state interest rate
RfreeSS = 1.0 - DeprFac + rFreeSS  # Steady state return factor
LivPrb = 1.0 - DiePrb  # Quarterly survival probability
DiscFacDSGE = RfreeSS ** (-1)  # Discount factor, HA-DSGE and RA models
TranShkVar = TranShkVarAnn * 4.0  # Variance of idiosyncratic transitory shocks
...
max_t_between_updates = (
    None  # Maximum number of periods an agent will go between updating (can be None)
)
...
DiscFacSetSOE = (
    Uniform(bot=DiscFacMeanSOE - DiscFacSpread, top=DiscFacMeanSOE + DiscFacSpread)
    .approx(N=TypeCount)
    .X
)
...
init_SOE_mrkv_market[
    "MrkvNow_init"
] = 0  # Start at 0 and stay there unless forced out by MIT shock
